  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <section className="rounded-2xl bg-slate-950 p-5 text-white shadow-sm">
        ...
        <h2 className="mt-2 text-2xl font-black md:text-3xl">Follow one token tensor through one self-attention + FFN block.</h2>
        <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">
          Start with the common shape-preserving core. Switch between encoder-style full self-attention and decoder-only
          causal self-attention. The encoder-decoder Decoder tab adds the separate cross-attention branch that is intentionally
          not included in this compact ledger.
        </p>
      </section>
        ...
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setMode('encoder')} className={`rounded-xl border px-3 py-2 text-sm font-black ${mode === 'encoder' ? 'border-blue-500 bg-blue-600 text-white' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>Encoder-style</button>
              <button type="button" onClick={() => setMode('decoder')} className={`rounded-xl border px-3 py-2 text-sm font-black ${mode === 'decoder' ? 'border-violet-500 bg-violet-600 text-white' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>Decoder-only</button>
            </div>
          </div>
        </aside>
