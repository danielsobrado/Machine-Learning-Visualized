...
import {
  PCA_P1_AUDITED_LESSON_IDS,
  PCA_P1_REQUIREMENTS,
} from './pcaP1Coverage.js';
...
test('PCA remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of PCA_P1_AUDITED_LESSON_IDS) {
...
test('PCA P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    PCA_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('PCA P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of PCA_P1_REQUIREMENTS) {
...
test('PCA P1 competencies require applied reasoning', () => {
  for (const requirement of PCA_P1_REQUIREMENTS) {
...
test('PCA scenario evidence remains tagged P1', () => {
...
test('PCA P1 contract keeps evidence independent', () => {
  const requirementIds = PCA_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    PCA_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...PCA_P1_AUDITED_LESSON_IDS].sort(),
