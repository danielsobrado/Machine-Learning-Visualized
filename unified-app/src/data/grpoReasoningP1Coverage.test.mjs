...
import {
  GRPO_REASONING_P1_AUDITED_LESSON_IDS,
  GRPO_REASONING_P1_REQUIREMENTS,
} from './grpoReasoningP1Coverage.js';
...
test('GRPO reasoning remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of GRPO_REASONING_P1_AUDITED_LESSON_IDS) {
...
test('GRPO reasoning P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    GRPO_REASONING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('GRPO reasoning P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of GRPO_REASONING_P1_REQUIREMENTS) {
...
test('GRPO reasoning P1 competencies require applied reasoning', () => {
  for (const requirement of GRPO_REASONING_P1_REQUIREMENTS) {
...
test('GRPO reasoning scenario evidence remains tagged P1', () => {
...
test('GRPO reasoning P1 contract keeps evidence independent', () => {
  const requirementIds = GRPO_REASONING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    GRPO_REASONING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...GRPO_REASONING_P1_AUDITED_LESSON_IDS].sort(),
