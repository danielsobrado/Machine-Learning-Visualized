...
import {
  TIME_SERIES_FORECASTING_P1_AUDITED_LESSON_IDS,
  TIME_SERIES_FORECASTING_P1_REQUIREMENTS,
} from './timeSeriesForecastingP1Coverage.js';
...
test('time series forecasting remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of TIME_SERIES_FORECASTING_P1_AUDITED_LESSON_IDS) {
...
test('time series forecasting P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    TIME_SERIES_FORECASTING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('time series forecasting P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of TIME_SERIES_FORECASTING_P1_REQUIREMENTS) {
...
test('time series forecasting P1 competencies require applied reasoning', () => {
  for (const requirement of TIME_SERIES_FORECASTING_P1_REQUIREMENTS) {
...
test('time series forecasting scenario evidence remains tagged P1', () => {
...
test('time series forecasting P1 contract keeps evidence independent', () => {
  const requirementIds = TIME_SERIES_FORECASTING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    TIME_SERIES_FORECASTING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...TIME_SERIES_FORECASTING_P1_AUDITED_LESSON_IDS].sort(),
