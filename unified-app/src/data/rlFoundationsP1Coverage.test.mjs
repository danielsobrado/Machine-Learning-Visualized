...
import {
  RL_FOUNDATIONS_P1_AUDITED_LESSON_IDS,
  RL_FOUNDATIONS_P1_REQUIREMENTS,
} from './rlFoundationsP1Coverage.js';
...
test('RL foundations remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RL_FOUNDATIONS_P1_AUDITED_LESSON_IDS) {
...
test('RL foundations P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RL_FOUNDATIONS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('RL foundations P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RL_FOUNDATIONS_P1_REQUIREMENTS) {
...
test('RL foundations P1 competencies require applied reasoning', () => {
  for (const requirement of RL_FOUNDATIONS_P1_REQUIREMENTS) {
...
test('RL foundations scenario evidence remains tagged P1', () => {
...
test('RL foundations P1 contract keeps evidence independent', () => {
  const requirementIds = RL_FOUNDATIONS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RL_FOUNDATIONS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RL_FOUNDATIONS_P1_AUDITED_LESSON_IDS].sort(),
