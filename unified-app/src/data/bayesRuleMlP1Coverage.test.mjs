...
import {
  BAYES_RULE_ML_P1_AUDITED_LESSON_IDS,
  BAYES_RULE_ML_P1_REQUIREMENTS,
} from './bayesRuleMlP1Coverage.js';
...
test('Bayes rule ML remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of BAYES_RULE_ML_P1_AUDITED_LESSON_IDS) {
...
test('Bayes rule ML P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    BAYES_RULE_ML_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('Bayes rule ML P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of BAYES_RULE_ML_P1_REQUIREMENTS) {
...
test('Bayes rule ML P1 competencies require applied reasoning', () => {
  for (const requirement of BAYES_RULE_ML_P1_REQUIREMENTS) {
...
test('Bayes rule ML scenario evidence remains tagged P1', () => {
...
test('Bayes rule ML P1 contract keeps evidence independent', () => {
  const requirementIds = BAYES_RULE_ML_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    BAYES_RULE_ML_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...BAYES_RULE_ML_P1_AUDITED_LESSON_IDS].sort(),
