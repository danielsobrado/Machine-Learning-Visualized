...
import {
  MAXIMUM_LIKELIHOOD_ESTIMATION_P1_AUDITED_LESSON_IDS,
  MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS,
} from './maximumLikelihoodEstimationP1Coverage.js';
...
test('maximum likelihood estimation remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of MAXIMUM_LIKELIHOOD_ESTIMATION_P1_AUDITED_LESSON_IDS) {
...
test('maximum likelihood estimation P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('maximum likelihood estimation P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS) {
...
test('maximum likelihood estimation P1 competencies require applied reasoning', () => {
  for (const requirement of MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS) {
...
test('maximum likelihood estimation scenario evidence remains tagged P1', () => {
...
test('maximum likelihood estimation P1 contract keeps evidence independent', () => {
  const requirementIds = MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    MAXIMUM_LIKELIHOOD_ESTIMATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...MAXIMUM_LIKELIHOOD_ESTIMATION_P1_AUDITED_LESSON_IDS].sort(),
