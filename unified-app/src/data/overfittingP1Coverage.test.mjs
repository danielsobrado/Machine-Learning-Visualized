...
import {
  OVERFITTING_P1_AUDITED_LESSON_IDS,
  OVERFITTING_P1_REQUIREMENTS,
} from './overfittingP1Coverage.js';
...
test('overfitting remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of OVERFITTING_P1_AUDITED_LESSON_IDS) {
...
test('overfitting P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    OVERFITTING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('overfitting P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of OVERFITTING_P1_REQUIREMENTS) {
...
test('overfitting P1 competencies require applied reasoning', () => {
  for (const requirement of OVERFITTING_P1_REQUIREMENTS) {
...
test('overfitting scenario evidence remains tagged P1', () => {
...
test('overfitting P1 contract keeps evidence independent', () => {
  const requirementIds = OVERFITTING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    OVERFITTING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...OVERFITTING_P1_AUDITED_LESSON_IDS].sort(),
