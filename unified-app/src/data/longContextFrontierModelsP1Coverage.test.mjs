...
import {
  LONG_CONTEXT_FRONTIER_MODELS_P1_AUDITED_LESSON_IDS,
  LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS,
} from './longContextFrontierModelsP1Coverage.js';
...
test('long-context frontier remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of LONG_CONTEXT_FRONTIER_MODELS_P1_AUDITED_LESSON_IDS) {
...
test('long-context frontier P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('long-context frontier P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS) {
...
test('long-context frontier P1 competencies require applied reasoning', () => {
  for (const requirement of LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS) {
...
test('long-context frontier scenario evidence remains tagged P1', () => {
...
test('long-context frontier P1 contract keeps evidence independent', () => {
  const requirementIds = LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    LONG_CONTEXT_FRONTIER_MODELS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...LONG_CONTEXT_FRONTIER_MODELS_P1_AUDITED_LESSON_IDS].sort(),
