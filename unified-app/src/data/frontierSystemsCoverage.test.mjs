...
import {
  FRONTIER_SYSTEMS_AUDITED_LESSON_IDS,
  FRONTIER_SYSTEMS_COVERAGE,
  FRONTIER_SYSTEMS_DEPTH_REQUIREMENTS,
} from './frontierSystemsCoverage.js';
import {
  getLessonAssessment,
...
function validateDepthScenario(requirementId, scenario) {
  ...
  assert.ok(
    ...
    `${requirementId}: ${scenario.id} explanation must show reasoning rather than only state the answer`,
  );
  assert.ok(
...
test('frontier systems audit remains aligned with priority assessment lessons', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of FRONTIER_SYSTEMS_AUDITED_LESSON_IDS) {
    assert.ok(priorityIds.has(lessonId), `${lessonId}: audited frontier lesson must remain a priority assessment`);
  }

  assert.equal(
    new Set(FRONTIER_SYSTEMS_AUDITED_LESSON_IDS).size,
    FRONTIER_SYSTEMS_AUDITED_LESSON_IDS.length,
    'audited frontier lesson ids must be unique',
  );
});

test('every audited frontier systems lesson remains covered', async (t) => {
  assert.deepEqual(
    Object.keys(FRONTIER_SYSTEMS_COVERAGE).sort(),
    [...FRONTIER_SYSTEMS_AUDITED_LESSON_IDS].sort(),
    'frontier topic coverage must stay aligned with the explicit audited lesson list',
  );

  for (const [lessonId, requirement] of Object.entries(FRONTIER_SYSTEMS_COVERAGE)) {
...
test('every audited frontier systems lesson has an explicit depth competency', () => {
  const coveredLessonIds = Object.keys(FRONTIER_SYSTEMS_COVERAGE).sort();
  const depthLessonIds = [...new Set(
    FRONTIER_SYSTEMS_DEPTH_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();

  assert.deepEqual(
    depthLessonIds,
    coveredLessonIds,
    'frontier topic coverage and depth coverage must stay aligned lesson-for-lesson',
  );
});

test('frontier systems depth competencies remain evidence-based', async (t) => {
  for (const requirement of FRONTIER_SYSTEMS_DEPTH_REQUIREMENTS) {
...
test('frontier systems depth set preserves multiple reasoning modes', () => {
  ...
  assert.ok(
    levels.size >= MIN_REASONING_MODES,
    `frontier systems depth should preserve at least ${MIN_REASONING_MODES} reasoning modes, got ${[...levels].join(', ')}`,
  );
});

test('frontier systems depth competencies are linked without scenario reuse', () => {
  const competencyIds = [];
  ...
  for (const requirement of FRONTIER_SYSTEMS_DEPTH_REQUIREMENTS) {
    ...
    assert.ok(
      FRONTIER_SYSTEMS_COVERAGE[requirement.lessonId],
      `${requirement.id}: lesson ${requirement.lessonId} must exist in frontier topic coverage`,
    );
    ...
    const coveredScenarioIds = new Set(FRONTIER_SYSTEMS_COVERAGE[requirement.lessonId].scenarioIds);
    for (const scenarioId of requirement.scenarioIds) {
      assert.ok(
        coveredScenarioIds.has(scenarioId),
        `${requirement.id}: ${scenarioId} must also be protected by frontier topic coverage`,
      );
      depthScenarioIds.push(scenarioId);
    }
  }

  assert.equal(new Set(competencyIds).size, competencyIds.length, 'frontier depth competency ids must be unique');
  assert.equal(
    new Set(depthScenarioIds).size,
    depthScenarioIds.length,
    'a frontier depth scenario should protect one explicit competency rather than be reused as a shortcut',
  );
});

test('frontier systems depth scenarios do not expose a dominant correct-answer position', () => {
  const counts = [0, 0, 0];
  ...
  assert.ok(total > 0, 'frontier depth scenarios should exist');
  ...
  assert.ok(
    ...
    `no frontier depth answer position should exceed ${MAX_DEPTH_ANSWER_SHARE * 100}% of scenarios, got ${counts.join(', ')}`,
  );
});

test('frontier systems coverage contract does not duplicate required ids within a lesson', () => {
  for (const [lessonId, requirement] of Object.entries(FRONTIER_SYSTEMS_COVERAGE)) {
    assert.equal(
      new Set(requirement.scenarioIds).size,
      requirement.scenarioIds.length,
