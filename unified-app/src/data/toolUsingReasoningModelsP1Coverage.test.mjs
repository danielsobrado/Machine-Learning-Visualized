...
import {
  TOOL_USING_REASONING_MODELS_P1_AUDITED_LESSON_IDS,
  TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS,
} from './toolUsingReasoningModelsP1Coverage.js';
...
test('tool-using reasoning models remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of TOOL_USING_REASONING_MODELS_P1_AUDITED_LESSON_IDS) {
...
test('tool-using reasoning models P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('tool-using reasoning models P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS) {
...
test('tool-using reasoning models P1 competencies require applied reasoning', () => {
  for (const requirement of TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS) {
...
test('tool-using reasoning models scenario evidence remains tagged P1', () => {
...
test('tool-using reasoning models P1 contract keeps evidence independent', () => {
  const requirementIds = TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    TOOL_USING_REASONING_MODELS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...TOOL_USING_REASONING_MODELS_P1_AUDITED_LESSON_IDS].sort(),
