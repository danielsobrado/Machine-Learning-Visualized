...
import {
  LAYER_NORMALIZATION_P1_AUDITED_LESSON_IDS,
  LAYER_NORMALIZATION_P1_REQUIREMENTS,
} from './layerNormalizationP1Coverage.js';
...
test('LayerNorm remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of LAYER_NORMALIZATION_P1_AUDITED_LESSON_IDS) {
...
test('LayerNorm P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    LAYER_NORMALIZATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('LayerNorm P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of LAYER_NORMALIZATION_P1_REQUIREMENTS) {
