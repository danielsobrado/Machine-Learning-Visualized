...
import {
  NEURAL_NETWORK_P1_AUDITED_LESSON_IDS,
  NEURAL_NETWORK_P1_REQUIREMENTS,
} from './neuralNetworkP1Coverage.js';
...
test('neural network remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of NEURAL_NETWORK_P1_AUDITED_LESSON_IDS) {
...
test('neural network P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    NEURAL_NETWORK_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('neural network P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of NEURAL_NETWORK_P1_REQUIREMENTS) {
