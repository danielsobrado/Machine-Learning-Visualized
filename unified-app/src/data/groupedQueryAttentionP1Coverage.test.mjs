...
import {
  GROUPED_QUERY_ATTENTION_P1_AUDITED_LESSON_IDS,
  GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS,
} from './groupedQueryAttentionP1Coverage.js';
...
test('grouped-query attention remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of GROUPED_QUERY_ATTENTION_P1_AUDITED_LESSON_IDS) {
...
test('grouped-query attention P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('grouped-query attention P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS) {
...
test('grouped-query attention P1 competencies require applied reasoning', () => {
  for (const requirement of GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS) {
...
test('grouped-query attention scenario evidence remains tagged P1', () => {
...
test('grouped-query attention P1 contract keeps evidence independent', () => {
  const requirementIds = GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    GROUPED_QUERY_ATTENTION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...GROUPED_QUERY_ATTENTION_P1_AUDITED_LESSON_IDS].sort(),
