...
import { TOKENIZATION_P1_AUDITED_LESSON_IDS, TOKENIZATION_P1_REQUIREMENTS } from './tokenizationP1Coverage.js';
...
test('tokenization remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of TOKENIZATION_P1_AUDITED_LESSON_IDS) {
...
test('tokenization P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(TOKENIZATION_P1_REQUIREMENTS.map(({ id }) => id).sort(), [...REQUIRED_COMPETENCY_IDS].sort());
});

test('tokenization P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of TOKENIZATION_P1_REQUIREMENTS) {
