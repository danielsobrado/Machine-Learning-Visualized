...
import {
  RECOMMENDER_SYSTEMS_RANKING_P1_AUDITED_LESSON_IDS,
  RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS,
} from './recommenderSystemsRankingP1Coverage.js';
...
test('recommender systems ranking remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RECOMMENDER_SYSTEMS_RANKING_P1_AUDITED_LESSON_IDS) {
...
test('recommender systems ranking P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('recommender systems ranking P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS) {
...
test('recommender systems ranking P1 competencies require applied reasoning', () => {
  for (const requirement of RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS) {
...
test('recommender systems ranking scenario evidence remains tagged P1', () => {
...
test('recommender systems ranking P1 contract keeps evidence independent', () => {
  const requirementIds = RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RECOMMENDER_SYSTEMS_RANKING_P1_AUDITED_LESSON_IDS].sort(),
  ...
  for (const requirement of RECOMMENDER_SYSTEMS_RANKING_P1_REQUIREMENTS) {
    ...
    assert.equal(new Set(requirementEvidenceIds).size, requirementEvidenceIds.length, `${requirement.id}: duplicate evidence id`);
