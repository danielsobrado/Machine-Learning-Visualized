...
import {
  RAG_AUDITED_LESSON_IDS,
  RAG_COVERAGE,
  RAG_DEPTH_REQUIREMENTS,
} from './ragCoverage.js';
import {
  getLessonAssessment,
...
test('RAG audit remains aligned with priority assessment lessons', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RAG_AUDITED_LESSON_IDS) {
    assert.ok(priorityIds.has(lessonId), `${lessonId}: audited RAG lesson must remain a priority assessment`);
  }

  assert.equal(
    new Set(RAG_AUDITED_LESSON_IDS).size,
    RAG_AUDITED_LESSON_IDS.length,
    'audited RAG lesson ids must be unique',
  );
});

test('every audited RAG lesson remains covered', async (t) => {
  const expectedLessonIds = [...RAG_AUDITED_LESSON_IDS].sort();
  assert.deepEqual(
    Object.keys(RAG_COVERAGE).sort(),
    expectedLessonIds,
    'RAG topic coverage must stay aligned with the explicit audited lesson list',
  );

  for (const [lessonId, requirement] of Object.entries(RAG_COVERAGE)) {
...
test('every audited RAG lesson has an explicit depth competency', () => {
  const coveredLessonIds = Object.keys(RAG_COVERAGE).sort();
  const depthLessonIds = [...new Set(
    RAG_DEPTH_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();

  assert.deepEqual(
    depthLessonIds,
    coveredLessonIds,
    'RAG topic coverage and depth coverage must stay aligned lesson-for-lesson',
  );
});

test('RAG depth competencies remain evidence-based', async (t) => {
  for (const requirement of RAG_DEPTH_REQUIREMENTS) {
...
test('RAG depth competencies are linked without scenario reuse', () => {
  const competencyIds = [];
  ...
  for (const requirement of RAG_DEPTH_REQUIREMENTS) {
    ...
    assert.ok(
      RAG_COVERAGE[requirement.lessonId],
      `${requirement.id}: lesson ${requirement.lessonId} must exist in RAG topic coverage`,
    );
    ...
    const coveredScenarioIds = new Set(RAG_COVERAGE[requirement.lessonId].scenarioIds);
    for (const scenarioId of requirement.scenarioIds) {
      assert.ok(
        coveredScenarioIds.has(scenarioId),
        `${requirement.id}: ${scenarioId} must also be protected by RAG topic coverage`,
      );
      depthScenarioIds.push(scenarioId);
    }
  }

  assert.equal(new Set(competencyIds).size, competencyIds.length, 'RAG depth competency ids must be unique');
  assert.equal(
    new Set(depthScenarioIds).size,
    depthScenarioIds.length,
    'a RAG depth scenario should protect one explicit competency rather than be reused as a shortcut',
  );
});

test('RAG depth scenarios do not expose a dominant correct-answer position', () => {
  const counts = [0, 0, 0];
  ...
  assert.ok(total > 0, 'RAG depth scenarios should exist');
  ...
  assert.ok(
    ...
    `no RAG depth answer position should exceed ${MAX_DEPTH_ANSWER_SHARE * 100}% of scenarios, got ${counts.join(', ')}`,
  );
});

test('RAG coverage contract does not duplicate required ids within a lesson', () => {
  for (const [lessonId, requirement] of Object.entries(RAG_COVERAGE)) {
    assert.equal(
      new Set(requirement.scenarioIds).size,
      requirement.scenarioIds.length,
