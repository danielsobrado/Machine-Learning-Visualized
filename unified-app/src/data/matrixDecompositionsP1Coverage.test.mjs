...
import {
  MATRIX_DECOMPOSITIONS_P1_AUDITED_LESSON_IDS,
  MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS,
} from './matrixDecompositionsP1Coverage.js';
...
test('matrix decompositions remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of MATRIX_DECOMPOSITIONS_P1_AUDITED_LESSON_IDS) {
...
test('matrix decompositions P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('matrix decompositions P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS) {
...
test('matrix decompositions P1 competencies require applied reasoning', () => {
  for (const requirement of MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS) {
...
test('matrix decompositions scenario evidence remains tagged P1', () => {
...
test('matrix decompositions P1 contract keeps evidence independent', () => {
  const requirementIds = MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    MATRIX_DECOMPOSITIONS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...MATRIX_DECOMPOSITIONS_P1_AUDITED_LESSON_IDS].sort(),
