...
import {
  NEURAL_NETWORK_AUDITED_LESSON_IDS,
  NEURAL_NETWORK_COVERAGE,
  NEURAL_NETWORK_DEPTH_REQUIREMENTS,
} from './neuralNetworkCoverage.js';
import {
  getLessonAssessment,
...
test('neural network audit remains aligned with priority assessment lessons', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of NEURAL_NETWORK_AUDITED_LESSON_IDS) {
    assert.ok(priorityIds.has(lessonId), `${lessonId}: audited neural lesson must remain a priority assessment`);
  }

  assert.equal(
    new Set(NEURAL_NETWORK_AUDITED_LESSON_IDS).size,
    NEURAL_NETWORK_AUDITED_LESSON_IDS.length,
    'audited neural lesson ids must be unique',
  );
});

test('every audited neural lesson remains covered', async (t) => {
  const expectedLessonIds = [...NEURAL_NETWORK_AUDITED_LESSON_IDS].sort();
  assert.deepEqual(
    Object.keys(NEURAL_NETWORK_COVERAGE).sort(),
    expectedLessonIds,
    'neural topic coverage must stay aligned with the explicit audited lesson list',
  );

  for (const [lessonId, requirement] of Object.entries(NEURAL_NETWORK_COVERAGE)) {
...
test('every audited neural lesson has an explicit depth competency', () => {
  const coveredLessonIds = Object.keys(NEURAL_NETWORK_COVERAGE).sort();
  const depthLessonIds = [...new Set(
    NEURAL_NETWORK_DEPTH_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();

  assert.deepEqual(
    depthLessonIds,
    coveredLessonIds,
    'neural topic coverage and depth coverage must stay aligned lesson-for-lesson',
  );
});

test('neural depth competencies remain evidence-based', async (t) => {
  for (const requirement of NEURAL_NETWORK_DEPTH_REQUIREMENTS) {
...
test('neural depth competencies are linked without scenario reuse', () => {
  const competencyIds = [];
  ...
  for (const requirement of NEURAL_NETWORK_DEPTH_REQUIREMENTS) {
    ...
    assert.ok(
      NEURAL_NETWORK_COVERAGE[requirement.lessonId],
      `${requirement.id}: lesson ${requirement.lessonId} must exist in neural topic coverage`,
    );
    ...
    const coveredScenarioIds = new Set(
      NEURAL_NETWORK_COVERAGE[requirement.lessonId].scenarioIds,
    );
    for (const scenarioId of requirement.scenarioIds) {
      assert.ok(
        coveredScenarioIds.has(scenarioId),
        `${requirement.id}: ${scenarioId} must also be protected by neural topic coverage`,
      );
      depthScenarioIds.push(scenarioId);
    }
  }

  assert.equal(new Set(competencyIds).size, competencyIds.length, 'neural depth competency ids must be unique');
  assert.equal(
    new Set(depthScenarioIds).size,
    depthScenarioIds.length,
    'a neural depth scenario should protect one explicit competency rather than be reused as a shortcut',
  );
});

test('neural depth scenarios do not expose a dominant correct-answer position', () => {
  const counts = [0, 0, 0];
  ...
  assert.ok(total > 0, 'neural depth scenarios should exist');
  ...
  assert.ok(
    ...
    `no neural depth answer position should exceed ${MAX_DEPTH_ANSWER_SHARE * 100}% of scenarios, got ${counts.join(', ')}`,
  );
});

test('neural coverage contract does not duplicate required ids within a lesson', () => {
  for (const [lessonId, requirement] of Object.entries(NEURAL_NETWORK_COVERAGE)) {
    assert.equal(
      new Set(requirement.scenarioIds).size,
      requirement.scenarioIds.length,
