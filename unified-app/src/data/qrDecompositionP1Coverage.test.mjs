...
import {
  QR_DECOMPOSITION_P1_AUDITED_LESSON_IDS,
  QR_DECOMPOSITION_P1_REQUIREMENTS,
} from './qrDecompositionP1Coverage.js';
...
test('QR decomposition remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of QR_DECOMPOSITION_P1_AUDITED_LESSON_IDS) {
...
test('QR decomposition P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    QR_DECOMPOSITION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('QR decomposition P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of QR_DECOMPOSITION_P1_REQUIREMENTS) {
...
test('QR decomposition P1 competencies require applied reasoning', () => {
  for (const requirement of QR_DECOMPOSITION_P1_REQUIREMENTS) {
...
test('QR decomposition scenario evidence remains tagged P1', () => {
...
test('QR decomposition P1 contract keeps evidence independent', () => {
  const requirementIds = QR_DECOMPOSITION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    QR_DECOMPOSITION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...QR_DECOMPOSITION_P1_AUDITED_LESSON_IDS].sort(),
