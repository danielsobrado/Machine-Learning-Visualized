...
import {
  GRADIENT_PROBLEMS_P1_AUDITED_LESSON_IDS,
  GRADIENT_PROBLEMS_P1_REQUIREMENTS,
} from './gradientProblemsP1Coverage.js';
...
test('gradient problems remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of GRADIENT_PROBLEMS_P1_AUDITED_LESSON_IDS) {
...
test('gradient problems P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    GRADIENT_PROBLEMS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('gradient problems P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of GRADIENT_PROBLEMS_P1_REQUIREMENTS) {
