...
import { MAX_POOLING_P1_AUDITED_LESSON_IDS, MAX_POOLING_P1_REQUIREMENTS } from './maxPoolingP1Coverage.js';

const DEPTH_SCENARIO_LEVELS = new Set([
  'application', 'calculation', 'comparison', 'decision', 'design', 'diagnosis', 'mechanism', 'visual-state',
]);
const REQUIRED_COMPETENCY_IDS = Object.freeze(['max-pooling-window-and-information-loss']);

function itemsById(items = []) {
  return new Map(items.map((item) => [item.id, item]));
}

test('max pooling remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of MAX_POOLING_P1_AUDITED_LESSON_IDS) {
...
test('max pooling P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(MAX_POOLING_P1_REQUIREMENTS.map(({ id }) => id).sort(), [...REQUIRED_COMPETENCY_IDS].sort());
});

test('max pooling P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of MAX_POOLING_P1_REQUIREMENTS) {
