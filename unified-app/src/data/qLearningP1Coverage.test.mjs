...
import {
  Q_LEARNING_P1_AUDITED_LESSON_IDS,
  Q_LEARNING_P1_REQUIREMENTS,
} from './qLearningP1Coverage.js';
...
test('Q-learning remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of Q_LEARNING_P1_AUDITED_LESSON_IDS) {
...
test('Q-learning P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    Q_LEARNING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('Q-learning P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of Q_LEARNING_P1_REQUIREMENTS) {
...
test('Q-learning P1 competencies require applied reasoning', () => {
  for (const requirement of Q_LEARNING_P1_REQUIREMENTS) {
...
test('Q-learning scenario evidence remains tagged P1', () => {
...
test('Q-learning P1 contract keeps evidence independent', () => {
  const requirementIds = Q_LEARNING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    Q_LEARNING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...Q_LEARNING_P1_AUDITED_LESSON_IDS].sort(),
