...
import {
  POWER_SAMPLE_SIZE_P1_AUDITED_LESSON_IDS,
  POWER_SAMPLE_SIZE_P1_REQUIREMENTS,
} from './powerSampleSizeP1Coverage.js';
...
test('power sample size remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of POWER_SAMPLE_SIZE_P1_AUDITED_LESSON_IDS) {
...
test('power sample size P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    POWER_SAMPLE_SIZE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('power sample size P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of POWER_SAMPLE_SIZE_P1_REQUIREMENTS) {
...
test('power sample size P1 competencies require applied reasoning', () => {
  for (const requirement of POWER_SAMPLE_SIZE_P1_REQUIREMENTS) {
...
test('power sample size scenario evidence remains tagged P1', () => {
...
test('power sample size P1 contract keeps evidence independent', () => {
  const requirementIds = POWER_SAMPLE_SIZE_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    POWER_SAMPLE_SIZE_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...POWER_SAMPLE_SIZE_P1_AUDITED_LESSON_IDS].sort(),
