...
import {
  KV_CACHE_P1_AUDITED_LESSON_IDS,
  KV_CACHE_P1_REQUIREMENTS,
} from './kvCacheP1Coverage.js';
...
test('KV cache remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of KV_CACHE_P1_AUDITED_LESSON_IDS) {
...
test('KV cache P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    KV_CACHE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('KV cache P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of KV_CACHE_P1_REQUIREMENTS) {
...
test('KV cache P1 competencies require applied reasoning', () => {
  for (const requirement of KV_CACHE_P1_REQUIREMENTS) {
...
test('KV cache scenario evidence remains tagged P1', () => {
...
test('KV cache P1 contract keeps evidence independent', () => {
  const requirementIds = KV_CACHE_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    KV_CACHE_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...KV_CACHE_P1_AUDITED_LESSON_IDS].sort(),
