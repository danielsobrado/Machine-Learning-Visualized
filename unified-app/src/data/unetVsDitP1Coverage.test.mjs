...
import {
  UNET_VS_DIT_P1_AUDITED_LESSON_IDS,
  UNET_VS_DIT_P1_REQUIREMENTS,
} from './unetVsDitP1Coverage.js';
...
test('U-Net vs DiT P1 lesson remains a priority curated assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of UNET_VS_DIT_P1_AUDITED_LESSON_IDS) {
...
test('U-Net vs DiT P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    UNET_VS_DIT_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('U-Net vs DiT P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of UNET_VS_DIT_P1_REQUIREMENTS) {
...
test('U-Net vs DiT P1 competencies require applied reasoning', () => {
  for (const requirement of UNET_VS_DIT_P1_REQUIREMENTS) {
...
test('U-Net vs DiT scenario evidence remains tagged P1', () => {
...
test('U-Net vs DiT P1 contract keeps evidence independent', () => {
  const requirementIds = UNET_VS_DIT_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    UNET_VS_DIT_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...UNET_VS_DIT_P1_AUDITED_LESSON_IDS].sort(),
