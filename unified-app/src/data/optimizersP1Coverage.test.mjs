...
import {
  OPTIMIZERS_P1_AUDITED_LESSON_IDS,
  OPTIMIZERS_P1_REQUIREMENTS,
} from './optimizersP1Coverage.js';
...
test('optimizers remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of OPTIMIZERS_P1_AUDITED_LESSON_IDS) {
...
test('optimizers P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    OPTIMIZERS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('optimizers P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of OPTIMIZERS_P1_REQUIREMENTS) {
