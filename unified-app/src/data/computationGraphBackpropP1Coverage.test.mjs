...
import {
  COMPUTATION_GRAPH_BACKPROP_P1_AUDITED_LESSON_IDS,
  COMPUTATION_GRAPH_BACKPROP_P1_REQUIREMENTS,
} from './computationGraphBackpropP1Coverage.js';
...
test('computation graph backprop remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of COMPUTATION_GRAPH_BACKPROP_P1_AUDITED_LESSON_IDS) {
...
test('computation graph backprop P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    COMPUTATION_GRAPH_BACKPROP_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('computation graph backprop P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of COMPUTATION_GRAPH_BACKPROP_P1_REQUIREMENTS) {
