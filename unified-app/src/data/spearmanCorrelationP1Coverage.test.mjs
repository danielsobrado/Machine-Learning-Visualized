...
import {
  SPEARMAN_CORRELATION_P1_AUDITED_LESSON_IDS,
  SPEARMAN_CORRELATION_P1_REQUIREMENTS,
} from './spearmanCorrelationP1Coverage.js';
...
test('spearman correlation remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of SPEARMAN_CORRELATION_P1_AUDITED_LESSON_IDS) {
...
test('spearman correlation P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    SPEARMAN_CORRELATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('spearman correlation P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of SPEARMAN_CORRELATION_P1_REQUIREMENTS) {
...
test('spearman correlation P1 competencies require applied reasoning', () => {
  for (const requirement of SPEARMAN_CORRELATION_P1_REQUIREMENTS) {
...
test('spearman correlation scenario evidence remains tagged P1', () => {
...
test('spearman correlation P1 contract keeps evidence independent', () => {
  const requirementIds = SPEARMAN_CORRELATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    SPEARMAN_CORRELATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...SPEARMAN_CORRELATION_P1_AUDITED_LESSON_IDS].sort(),
