...
import {
  COSINE_SIMILARITY_P1_AUDITED_LESSON_IDS,
  COSINE_SIMILARITY_P1_REQUIREMENTS,
} from './cosineSimilarityP1Coverage.js';
...
test('cosine similarity remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of COSINE_SIMILARITY_P1_AUDITED_LESSON_IDS) {
...
test('cosine similarity P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(COSINE_SIMILARITY_P1_REQUIREMENTS.map(({ id }) => id).sort(), [...REQUIRED_COMPETENCY_IDS].sort());
});

test('cosine similarity P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of COSINE_SIMILARITY_P1_REQUIREMENTS) {
