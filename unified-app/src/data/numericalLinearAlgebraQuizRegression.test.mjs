...
import {
  NUMERICAL_LINEAR_ALGEBRA_QUIZ_AUDITED_LESSON_IDS,
  NUMERICAL_LINEAR_ALGEBRA_QUIZ_REQUIREMENTS,
} from './numericalLinearAlgebraQuizRegression.js';

function assertRegistryCoverage() {
  const requirementLessonIds = [...new Set(NUMERICAL_LINEAR_ALGEBRA_QUIZ_REQUIREMENTS.map(({ lessonId }) => lessonId))].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...NUMERICAL_LINEAR_ALGEBRA_QUIZ_AUDITED_LESSON_IDS].sort(),
    'every audited lesson must have at least one explicit competency contract',
  );
}

function assertUniqueEvidenceIds() {
  const evidenceIds = NUMERICAL_LINEAR_ALGEBRA_QUIZ_REQUIREMENTS.flatMap(({ quizIds }) => quizIds);
  assert.equal(
...
test('numerical linear algebra audited lessons remain complete curated assessments', () => {
  for (const lessonId of NUMERICAL_LINEAR_ALGEBRA_QUIZ_AUDITED_LESSON_IDS) {
...
test('numerical linear algebra competencies retain explicit reasoning evidence', async (t) => {
  for (const requirement of NUMERICAL_LINEAR_ALGEBRA_QUIZ_REQUIREMENTS) {
