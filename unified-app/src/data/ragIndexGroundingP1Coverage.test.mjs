...
import {
  RAG_INDEX_GROUNDING_P1_AUDITED_LESSON_IDS,
  RAG_INDEX_GROUNDING_P1_REQUIREMENTS,
} from './ragIndexGroundingP1Coverage.js';
...
test('RAG indexing and grounding P1 lessons remain priority curated assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RAG_INDEX_GROUNDING_P1_AUDITED_LESSON_IDS) {
...
test('RAG indexing and grounding P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RAG_INDEX_GROUNDING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('RAG indexing and grounding P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RAG_INDEX_GROUNDING_P1_REQUIREMENTS) {
...
test('RAG indexing and grounding P1 competencies require applied reasoning', () => {
  for (const requirement of RAG_INDEX_GROUNDING_P1_REQUIREMENTS) {
...
test('RAG indexing and grounding scenario evidence remains tagged P1', () => {
...
test('RAG indexing and grounding P1 contract keeps evidence independent', () => {
  const requirementIds = RAG_INDEX_GROUNDING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RAG_INDEX_GROUNDING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RAG_INDEX_GROUNDING_P1_AUDITED_LESSON_IDS].sort(),
