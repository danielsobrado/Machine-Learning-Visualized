...
import {
  RL_EXPLORATION_P1_AUDITED_LESSON_IDS,
  RL_EXPLORATION_P1_REQUIREMENTS,
} from './rlExplorationP1Coverage.js';
...
test('RL exploration remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RL_EXPLORATION_P1_AUDITED_LESSON_IDS) {
...
test('RL exploration P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RL_EXPLORATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('RL exploration P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RL_EXPLORATION_P1_REQUIREMENTS) {
...
test('RL exploration P1 competencies require applied reasoning', () => {
  for (const requirement of RL_EXPLORATION_P1_REQUIREMENTS) {
...
test('RL exploration scenario evidence remains tagged P1', () => {
...
test('RL exploration P1 contract keeps evidence independent', () => {
  const requirementIds = RL_EXPLORATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RL_EXPLORATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RL_EXPLORATION_P1_AUDITED_LESSON_IDS].sort(),
