...
import {
  FEATURE_SCALING_PREPROCESSING_P1_AUDITED_LESSON_IDS,
  FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS,
} from './featureScalingPreprocessingP1Coverage.js';
...
test('feature scaling remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of FEATURE_SCALING_PREPROCESSING_P1_AUDITED_LESSON_IDS) {
...
test('feature scaling P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('feature scaling P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS) {
...
test('feature scaling P1 competencies require applied reasoning', () => {
  for (const requirement of FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS) {
...
test('feature scaling scenario evidence remains tagged P1', () => {
...
test('feature scaling P1 contract keeps evidence independent', () => {
  const requirementIds = FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    FEATURE_SCALING_PREPROCESSING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...FEATURE_SCALING_PREPROCESSING_P1_AUDITED_LESSON_IDS].sort(),
