...
import {
  REINFORCEMENT_LEARNING_CORE_P1_AUDITED_LESSON_IDS,
  REINFORCEMENT_LEARNING_CORE_P1_REQUIREMENTS,
} from './reinforcementLearningCoreP1Coverage.js';
...
test('reinforcement-learning core lessons remain priority curated P1 assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of REINFORCEMENT_LEARNING_CORE_P1_AUDITED_LESSON_IDS) {
...
test('reinforcement-learning core P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    REINFORCEMENT_LEARNING_CORE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('reinforcement-learning core P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of REINFORCEMENT_LEARNING_CORE_P1_REQUIREMENTS) {
