...
import {
  CLASSIFIER_FREE_GUIDANCE_P1_AUDITED_LESSON_IDS,
  CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS,
} from './classifierFreeGuidanceP1Coverage.js';
...
test('classifier-free guidance remains a priority curated assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of CLASSIFIER_FREE_GUIDANCE_P1_AUDITED_LESSON_IDS) {
...
test('classifier-free guidance P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('classifier-free guidance P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS) {
...
test('classifier-free guidance P1 competencies require applied reasoning', () => {
  for (const requirement of CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS) {
...
test('classifier-free guidance scenario evidence remains tagged P1', () => {
...
test('classifier-free guidance P1 contract keeps evidence independent', () => {
  const requirementIds = CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    CLASSIFIER_FREE_GUIDANCE_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...CLASSIFIER_FREE_GUIDANCE_P1_AUDITED_LESSON_IDS].sort(),
