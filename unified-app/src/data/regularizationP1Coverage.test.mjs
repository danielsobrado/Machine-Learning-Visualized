...
import {
  REGULARIZATION_P1_AUDITED_LESSON_IDS,
  REGULARIZATION_P1_REQUIREMENTS,
} from './regularizationP1Coverage.js';
...
test('regularization remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of REGULARIZATION_P1_AUDITED_LESSON_IDS) {
...
test('regularization P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    REGULARIZATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('regularization P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of REGULARIZATION_P1_REQUIREMENTS) {
...
test('regularization P1 competencies require applied reasoning', () => {
  for (const requirement of REGULARIZATION_P1_REQUIREMENTS) {
...
test('regularization scenario evidence remains tagged P1', () => {
...
test('regularization P1 contract keeps evidence independent', () => {
  const requirementIds = REGULARIZATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    REGULARIZATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...REGULARIZATION_P1_AUDITED_LESSON_IDS].sort(),
