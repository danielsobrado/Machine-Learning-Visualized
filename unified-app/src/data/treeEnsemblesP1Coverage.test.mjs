...
import {
  TREE_ENSEMBLES_P1_AUDITED_LESSON_IDS,
  TREE_ENSEMBLES_P1_REQUIREMENTS,
} from './treeEnsemblesP1Coverage.js';
...
test('tree ensembles remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of TREE_ENSEMBLES_P1_AUDITED_LESSON_IDS) {
...
test('tree ensembles P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    TREE_ENSEMBLES_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('tree ensembles P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of TREE_ENSEMBLES_P1_REQUIREMENTS) {
...
test('tree ensembles P1 competencies require applied reasoning', () => {
  for (const requirement of TREE_ENSEMBLES_P1_REQUIREMENTS) {
...
test('tree ensembles scenario evidence remains tagged P1', () => {
...
test('tree ensembles P1 contract keeps evidence independent', () => {
  const requirementIds = TREE_ENSEMBLES_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    TREE_ENSEMBLES_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...TREE_ENSEMBLES_P1_AUDITED_LESSON_IDS].sort(),
