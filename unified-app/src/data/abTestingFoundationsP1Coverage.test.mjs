...
import {
  AB_TESTING_FOUNDATIONS_P1_AUDITED_LESSON_IDS,
  AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS,
} from './abTestingFoundationsP1Coverage.js';
...
test('ab testing foundations remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of AB_TESTING_FOUNDATIONS_P1_AUDITED_LESSON_IDS) {
...
test('ab testing foundations P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('ab testing foundations P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS) {
...
test('ab testing foundations P1 competencies require applied reasoning', () => {
  for (const requirement of AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS) {
...
test('ab testing foundations scenario evidence remains tagged P1', () => {
...
test('ab testing foundations P1 contract keeps evidence independent', () => {
  const requirementIds = AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    AB_TESTING_FOUNDATIONS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...AB_TESTING_FOUNDATIONS_P1_AUDITED_LESSON_IDS].sort(),
