...
import {
  RESIDUAL_STREAM_P1_AUDITED_LESSON_IDS,
  RESIDUAL_STREAM_P1_REQUIREMENTS,
} from './residualStreamP1Coverage.js';
...
test('residual stream remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RESIDUAL_STREAM_P1_AUDITED_LESSON_IDS) {
...
test('residual stream P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RESIDUAL_STREAM_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('residual stream P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RESIDUAL_STREAM_P1_REQUIREMENTS) {
...
test('residual stream P1 competencies require applied reasoning', () => {
  for (const requirement of RESIDUAL_STREAM_P1_REQUIREMENTS) {
...
test('residual stream scenario evidence remains tagged P1', () => {
...
test('residual stream P1 contract keeps evidence independent', () => {
  const requirementIds = RESIDUAL_STREAM_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RESIDUAL_STREAM_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RESIDUAL_STREAM_P1_AUDITED_LESSON_IDS].sort(),
