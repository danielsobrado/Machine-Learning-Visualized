...
import {
  LOGISTIC_REGRESSION_P1_AUDITED_LESSON_IDS,
  LOGISTIC_REGRESSION_P1_REQUIREMENTS,
} from './logisticRegressionP1Coverage.js';
...
test('logistic regression remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of LOGISTIC_REGRESSION_P1_AUDITED_LESSON_IDS) {
...
test('logistic regression P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    LOGISTIC_REGRESSION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('logistic regression P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of LOGISTIC_REGRESSION_P1_REQUIREMENTS) {
...
test('logistic regression P1 competencies require applied reasoning', () => {
  for (const requirement of LOGISTIC_REGRESSION_P1_REQUIREMENTS) {
...
test('logistic regression scenario evidence remains tagged P1', () => {
...
test('logistic regression P1 contract keeps evidence independent', () => {
  const requirementIds = LOGISTIC_REGRESSION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    LOGISTIC_REGRESSION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...LOGISTIC_REGRESSION_P1_AUDITED_LESSON_IDS].sort(),
