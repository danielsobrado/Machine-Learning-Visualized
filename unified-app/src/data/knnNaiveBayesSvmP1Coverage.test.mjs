...
import {
  KNN_NAIVE_BAYES_SVM_P1_AUDITED_LESSON_IDS,
  KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS,
} from './knnNaiveBayesSvmP1Coverage.js';
...
test('kNN Naive Bayes SVM remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of KNN_NAIVE_BAYES_SVM_P1_AUDITED_LESSON_IDS) {
...
test('kNN Naive Bayes SVM P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('kNN Naive Bayes SVM P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS) {
...
test('kNN Naive Bayes SVM P1 competencies require applied reasoning', () => {
  for (const requirement of KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS) {
...
test('kNN Naive Bayes SVM scenario evidence remains tagged P1', () => {
...
test('kNN Naive Bayes SVM P1 contract keeps evidence independent', () => {
  const requirementIds = KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    KNN_NAIVE_BAYES_SVM_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...KNN_NAIVE_BAYES_SVM_P1_AUDITED_LESSON_IDS].sort(),
