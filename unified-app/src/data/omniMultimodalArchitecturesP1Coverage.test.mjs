...
import {
  OMNI_MULTIMODAL_ARCHITECTURES_P1_AUDITED_LESSON_IDS,
  OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS,
} from './omniMultimodalArchitecturesP1Coverage.js';
...
test('omni multimodal architectures remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of OMNI_MULTIMODAL_ARCHITECTURES_P1_AUDITED_LESSON_IDS) {
...
test('omni multimodal architectures P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('omni multimodal architectures P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS) {
...
test('omni multimodal architectures P1 competencies require applied reasoning', () => {
  for (const requirement of OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS) {
...
test('omni multimodal architectures scenario evidence remains tagged P1', () => {
...
test('omni multimodal architectures P1 contract keeps evidence independent', () => {
  const requirementIds = OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    OMNI_MULTIMODAL_ARCHITECTURES_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...OMNI_MULTIMODAL_ARCHITECTURES_P1_AUDITED_LESSON_IDS].sort(),
