...
import {
  HYPOTHESIS_TESTING_INTUITION_P1_AUDITED_LESSON_IDS,
  HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS,
} from './hypothesisTestingIntuitionP1Coverage.js';
...
test('hypothesis testing intuition remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of HYPOTHESIS_TESTING_INTUITION_P1_AUDITED_LESSON_IDS) {
...
test('hypothesis testing intuition P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('hypothesis testing intuition P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS) {
...
test('hypothesis testing intuition P1 competencies require applied reasoning', () => {
  for (const requirement of HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS) {
...
test('hypothesis testing intuition scenario evidence remains tagged P1', () => {
...
test('hypothesis testing intuition P1 contract keeps evidence independent', () => {
  const requirementIds = HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    HYPOTHESIS_TESTING_INTUITION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...HYPOTHESIS_TESTING_INTUITION_P1_AUDITED_LESSON_IDS].sort(),
