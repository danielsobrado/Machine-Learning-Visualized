...
import {
  CALIBRATION_P1_AUDITED_LESSON_IDS,
  CALIBRATION_P1_REQUIREMENTS,
} from './calibrationP1Coverage.js';
...
test('calibration remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of CALIBRATION_P1_AUDITED_LESSON_IDS) {
...
test('calibration P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    CALIBRATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('calibration P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of CALIBRATION_P1_REQUIREMENTS) {
...
test('calibration P1 competencies require applied reasoning', () => {
  for (const requirement of CALIBRATION_P1_REQUIREMENTS) {
...
test('calibration scenario evidence remains tagged P1', () => {
...
test('calibration P1 contract keeps evidence independent', () => {
  const requirementIds = CALIBRATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    CALIBRATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...CALIBRATION_P1_AUDITED_LESSON_IDS].sort(),
