...
import {
  INITIALIZATION_P1_AUDITED_LESSON_IDS,
  INITIALIZATION_P1_REQUIREMENTS,
} from './initializationP1Coverage.js';
...
test('initialization remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of INITIALIZATION_P1_AUDITED_LESSON_IDS) {
...
test('initialization P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    INITIALIZATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('initialization P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of INITIALIZATION_P1_REQUIREMENTS) {
