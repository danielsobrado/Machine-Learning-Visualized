...
import {
  PROPENSITY_SCORES_P1_AUDITED_LESSON_IDS,
  PROPENSITY_SCORES_P1_REQUIREMENTS,
} from './propensityScoresP1Coverage.js';
...
test('propensity scores remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of PROPENSITY_SCORES_P1_AUDITED_LESSON_IDS) {
...
test('propensity scores P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    PROPENSITY_SCORES_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('propensity scores P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of PROPENSITY_SCORES_P1_REQUIREMENTS) {
...
test('propensity scores P1 competencies require applied reasoning', () => {
  for (const requirement of PROPENSITY_SCORES_P1_REQUIREMENTS) {
...
test('propensity scores scenario evidence remains tagged P1', () => {
...
test('propensity scores P1 contract keeps evidence independent', () => {
  const requirementIds = PROPENSITY_SCORES_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    PROPENSITY_SCORES_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...PROPENSITY_SCORES_P1_AUDITED_LESSON_IDS].sort(),
