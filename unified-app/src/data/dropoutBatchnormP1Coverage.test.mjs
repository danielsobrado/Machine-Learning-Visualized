...
import {
  DROPOUT_BATCHNORM_P1_AUDITED_LESSON_IDS,
  DROPOUT_BATCHNORM_P1_REQUIREMENTS,
} from './dropoutBatchnormP1Coverage.js';
...
test('dropout and BatchNorm remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of DROPOUT_BATCHNORM_P1_AUDITED_LESSON_IDS) {
...
test('dropout and BatchNorm P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    DROPOUT_BATCHNORM_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('dropout and BatchNorm P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of DROPOUT_BATCHNORM_P1_REQUIREMENTS) {
