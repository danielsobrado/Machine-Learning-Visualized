...
import {
  FUNDAMENTAL_SUBSPACES_P1_AUDITED_LESSON_IDS,
  FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS,
} from './fundamentalSubspacesP1Coverage.js';
...
test('fundamental subspaces remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of FUNDAMENTAL_SUBSPACES_P1_AUDITED_LESSON_IDS) {
...
test('fundamental subspaces P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('fundamental subspaces P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS) {
...
test('fundamental subspaces P1 competencies require applied reasoning', () => {
  for (const requirement of FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS) {
...
test('fundamental subspaces scenario evidence remains tagged P1', () => {
...
test('fundamental subspaces P1 contract keeps evidence independent', () => {
  const requirementIds = FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    FUNDAMENTAL_SUBSPACES_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...FUNDAMENTAL_SUBSPACES_P1_AUDITED_LESSON_IDS].sort(),
