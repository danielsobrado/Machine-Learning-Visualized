...
import {
  RAG_FAILURE_EVALUATION_P1_AUDITED_LESSON_IDS,
  RAG_FAILURE_EVALUATION_P1_REQUIREMENTS,
} from './ragFailureEvaluationP1Coverage.js';
...
test('RAG failure and evaluation P1 lessons remain priority curated assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of RAG_FAILURE_EVALUATION_P1_AUDITED_LESSON_IDS) {
...
test('RAG failure and evaluation P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    RAG_FAILURE_EVALUATION_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('RAG failure and evaluation P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of RAG_FAILURE_EVALUATION_P1_REQUIREMENTS) {
...
test('RAG failure and evaluation P1 competencies require applied reasoning', () => {
  for (const requirement of RAG_FAILURE_EVALUATION_P1_REQUIREMENTS) {
...
test('RAG failure and evaluation scenario evidence remains tagged P1', () => {
...
test('RAG failure and evaluation P1 contract keeps evidence independent', () => {
  const requirementIds = RAG_FAILURE_EVALUATION_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    RAG_FAILURE_EVALUATION_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...RAG_FAILURE_EVALUATION_P1_AUDITED_LESSON_IDS].sort(),
