...
import {
  REASONING_RLVR_GRPO_P1_AUDITED_LESSON_IDS,
  REASONING_RLVR_GRPO_P1_REQUIREMENTS,
} from './reasoningRlvrGrpoP1Coverage.js';
...
test('RLVR GRPO remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of REASONING_RLVR_GRPO_P1_AUDITED_LESSON_IDS) {
...
test('RLVR GRPO P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    REASONING_RLVR_GRPO_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('RLVR GRPO P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of REASONING_RLVR_GRPO_P1_REQUIREMENTS) {
...
test('RLVR GRPO P1 competencies require applied reasoning', () => {
  for (const requirement of REASONING_RLVR_GRPO_P1_REQUIREMENTS) {
...
test('RLVR GRPO scenario evidence remains tagged P1', () => {
...
test('RLVR GRPO P1 contract keeps evidence independent', () => {
  const requirementIds = REASONING_RLVR_GRPO_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    REASONING_RLVR_GRPO_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...REASONING_RLVR_GRPO_P1_AUDITED_LESSON_IDS].sort(),
