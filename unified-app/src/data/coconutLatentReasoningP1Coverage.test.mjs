...
import {
  COCONUT_LATENT_REASONING_P1_AUDITED_LESSON_IDS,
  COCONUT_LATENT_REASONING_P1_REQUIREMENTS,
} from './coconutLatentReasoningP1Coverage.js';
...
test('Coconut latent reasoning remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of COCONUT_LATENT_REASONING_P1_AUDITED_LESSON_IDS) {
...
test('Coconut latent reasoning P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    COCONUT_LATENT_REASONING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('Coconut latent reasoning P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of COCONUT_LATENT_REASONING_P1_REQUIREMENTS) {
...
test('Coconut latent reasoning P1 competencies require applied reasoning', () => {
  for (const requirement of COCONUT_LATENT_REASONING_P1_REQUIREMENTS) {
...
test('Coconut latent reasoning scenario evidence remains tagged P1', () => {
...
test('Coconut latent reasoning P1 contract keeps evidence independent', () => {
  const requirementIds = COCONUT_LATENT_REASONING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    COCONUT_LATENT_REASONING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...COCONUT_LATENT_REASONING_P1_AUDITED_LESSON_IDS].sort(),
