...
import {
  SAMPLING_CONFIDENCE_INTERVALS_P1_AUDITED_LESSON_IDS,
  SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS,
} from './samplingConfidenceIntervalsP1Coverage.js';
...
test('sampling confidence intervals remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of SAMPLING_CONFIDENCE_INTERVALS_P1_AUDITED_LESSON_IDS) {
...
test('sampling confidence intervals P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('sampling confidence intervals P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS) {
...
test('sampling confidence intervals P1 competencies require applied reasoning', () => {
  for (const requirement of SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS) {
...
test('sampling confidence intervals scenario evidence remains tagged P1', () => {
...
test('sampling confidence intervals P1 contract keeps evidence independent', () => {
  const requirementIds = SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    SAMPLING_CONFIDENCE_INTERVALS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...SAMPLING_CONFIDENCE_INTERVALS_P1_AUDITED_LESSON_IDS].sort(),
