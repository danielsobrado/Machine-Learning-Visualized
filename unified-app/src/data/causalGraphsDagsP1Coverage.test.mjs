...
import {
  CAUSAL_GRAPHS_DAGS_P1_AUDITED_LESSON_IDS,
  CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS,
} from './causalGraphsDagsP1Coverage.js';
...
test('causal graphs DAGs remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of CAUSAL_GRAPHS_DAGS_P1_AUDITED_LESSON_IDS) {
...
test('causal graphs DAGs P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('causal graphs DAGs P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS) {
...
test('causal graphs DAGs P1 competencies require applied reasoning', () => {
  for (const requirement of CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS) {
...
test('causal graphs DAGs scenario evidence remains tagged P1', () => {
...
test('causal graphs DAGs P1 contract keeps evidence independent', () => {
  const requirementIds = CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    CAUSAL_GRAPHS_DAGS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...CAUSAL_GRAPHS_DAGS_P1_AUDITED_LESSON_IDS].sort(),
