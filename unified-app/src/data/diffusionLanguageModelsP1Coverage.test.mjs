...
import {
  DIFFUSION_LANGUAGE_MODELS_P1_AUDITED_LESSON_IDS,
  DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS,
} from './diffusionLanguageModelsP1Coverage.js';
...
test('diffusion language models remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of DIFFUSION_LANGUAGE_MODELS_P1_AUDITED_LESSON_IDS) {
...
test('diffusion language models P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('diffusion language models P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS) {
...
test('diffusion language models P1 competencies require applied reasoning', () => {
  for (const requirement of DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS) {
...
test('diffusion language model scenario evidence remains tagged P1', () => {
...
test('diffusion language models P1 contract keeps evidence independent', () => {
  const requirementIds = DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    DIFFUSION_LANGUAGE_MODELS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...DIFFUSION_LANGUAGE_MODELS_P1_AUDITED_LESSON_IDS].sort(),
