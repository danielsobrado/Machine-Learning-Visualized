...
import {
  ROC_PR_CURVES_P1_AUDITED_LESSON_IDS,
  ROC_PR_CURVES_P1_REQUIREMENTS,
} from './rocPrCurvesP1Coverage.js';
...
test('ROC PR curves remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of ROC_PR_CURVES_P1_AUDITED_LESSON_IDS) {
...
test('ROC PR curves P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    ROC_PR_CURVES_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('ROC PR curves P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of ROC_PR_CURVES_P1_REQUIREMENTS) {
...
test('ROC PR curves P1 competencies require applied reasoning', () => {
  for (const requirement of ROC_PR_CURVES_P1_REQUIREMENTS) {
...
test('ROC PR curves scenario evidence remains tagged P1', () => {
...
test('ROC PR curves P1 contract keeps evidence independent', () => {
  const requirementIds = ROC_PR_CURVES_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    ROC_PR_CURVES_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...ROC_PR_CURVES_P1_AUDITED_LESSON_IDS].sort(),
