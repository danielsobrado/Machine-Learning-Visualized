...
import {
  DAPO_REASONING_P1_AUDITED_LESSON_IDS,
  DAPO_REASONING_P1_REQUIREMENTS,
} from './dapoReasoningP1Coverage.js';
...
test('DAPO reasoning remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of DAPO_REASONING_P1_AUDITED_LESSON_IDS) {
...
test('DAPO reasoning P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    DAPO_REASONING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('DAPO reasoning P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of DAPO_REASONING_P1_REQUIREMENTS) {
...
test('DAPO reasoning P1 competencies require applied reasoning', () => {
  for (const requirement of DAPO_REASONING_P1_REQUIREMENTS) {
...
test('DAPO reasoning scenario evidence remains tagged P1', () => {
...
test('DAPO reasoning P1 contract keeps evidence independent', () => {
  const requirementIds = DAPO_REASONING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    DAPO_REASONING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...DAPO_REASONING_P1_AUDITED_LESSON_IDS].sort(),
