...
import {
  FRONTIER_SYSTEMS_CORE_P1_AUDITED_LESSON_IDS,
  FRONTIER_SYSTEMS_CORE_P1_REQUIREMENTS,
} from './frontierSystemsCoreP1Coverage.js';
...
test('frontier-system core lessons remain priority curated P1 assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of FRONTIER_SYSTEMS_CORE_P1_AUDITED_LESSON_IDS) {
...
test('frontier-system core P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    FRONTIER_SYSTEMS_CORE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('frontier-system core P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of FRONTIER_SYSTEMS_CORE_P1_REQUIREMENTS) {
