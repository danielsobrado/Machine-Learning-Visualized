...
import { RELU_P1_AUDITED_LESSON_IDS, RELU_P1_REQUIREMENTS } from './reluP1Coverage.js';
...
test('ReLU remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of RELU_P1_AUDITED_LESSON_IDS) {
...
test('ReLU P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(RELU_P1_REQUIREMENTS.map(({ id }) => id).sort(), [...REQUIRED_COMPETENCY_IDS].sort());
});

test('ReLU P1 competencies resolve to live applied P1 evidence', async (t) => {
  const priorityByScenarioId = new Map(
    getAssessmentScenarioExtensionEntries().map(({ priority, question }) => [question.id, priority]),
  );
  const evidenceIds = [];

  for (const requirement of RELU_P1_REQUIREMENTS) {
