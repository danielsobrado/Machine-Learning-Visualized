...
export const categories = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'coconut-latent-reasoning', name: 'Coconut: Chain of Continuous Thought', icon: Brain, description: 'Reasoning in hidden-state space instead of language tokens' },
      ...
      {
        id: 'spec-sparse-attention',
        name: 'SpecSA / SpecAttn: Draft Many Tokens, Read Fewer KV Blocks',
        icon: GitBranch,
        description: 'How speculative decoding and sparse attention work together without destroying KV-block reuse',
      },
      {
        id: 'turboquant',
        name: 'TurboQuant: Shrinking the KV Cache Without Forgetting',
        icon: Binary,
        description: 'Low-bit KV-cache compression that preserves attention dot products',
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: 'dapo-reasoning-rl',
        name: 'DAPO: Fixing GRPO at Scale',
        icon: BookOpen,
        description: 'A paper lesson on Dynamic Sampling, Clip-Higher, token-level loss, overlong reward shaping, and mini-dapo Rust exercises',
      },
      {
        id: 'coconut-latent-reasoning',
        name: 'Coconut: Reasoning Between Tokens',
        icon: BookOpen,
        description: 'A paper lesson on chain of continuous thought, hidden-state feedback, latent reasoning curricula, delayed commitment, and faithfulness tests',
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        id: 'dapo-reasoning-rl',
        name: 'DAPO: Making Reasoning RL Actually Train',
        icon: Cpu,
        description: 'Dynamic Sampling, Clip-Higher, token-level loss, and overlong reward shaping for long-CoT reasoning RL',
      },
      {
      ...
      {
        id: 'coconut-latent-reasoning',
        name: 'Coconut: Latent Reasoning Between Tokens',
        icon: Cpu,
        description: 'Feed hidden states back as continuous thoughts, then test delayed commitment and faithfulness risks',
      },
      {
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'dapo-reasoning-rl', name: 'DAPO: Fixing GRPO at Scale', icon: Brain, description: 'Practical fixes for long-CoT group-relative RL: useful batches, Clip-Higher, token credit, and length shaping' },
...
const CURRICULUM_OVERRIDES = {
  ...
  'dapo-reasoning-rl': {
    ...
    learningObjectives: [
      'Explain why all-correct and all-wrong GRPO groups waste rollout and optimizer budget',
      'Use Dynamic Sampling to keep prompt groups with mixed success and useful gradients',
      'Contrast symmetric PPO clipping with Clip-Higher asymmetric clipping for positive-advantage updates',
      'Describe why token-level policy-gradient loss gives denser credit for long chain-of-thought traces',
      'Compare hard overlong penalties with shaped length penalties near the response limit',
      'Diagnose long-CoT RL health from effective groups, entropy, clip fraction, overlong rate, and token-gradient count',
    ],
    commonMisconception: 'DAPO is not just GRPO with a new name; it is a recipe where batch filtering, asymmetric clipping, token-level loss aggregation, and length reward shaping interact.',
  },
  'coconut-latent-reasoning': {
    difficulty: 'advanced',
    estimatedMinutes: 55,
    ...
    learningObjectives: [
      'Contrast text chain-of-thought with continuous hidden-state reasoning',
      'Trace language mode versus latent mode and explain why the LM head is bypassed',
      'Explain the curriculum that replaces early text reasoning steps with latent thoughts',
      'Visualize delayed commitment as branch entropy falling across latent steps',
      'Evaluate faithfulness risks with perturbations, probes, ablations, and shortcut tests',
    ],
    commonMisconception: 'A latent thought is not automatically faithful or interpretable; it may help prediction while remaining opaque or shortcut-driven.',
  },
  'reasoning-rlvr-grpo': {
    difficulty: 'advanced',
    estimatedMinutes: 60,
  ...
  'spec-sparse-attention': {
    difficulty: 'advanced',
    estimatedMinutes: 40,
    prerequisites: ['eagle-3-1-speculative-decoding', 'kv-cache', 'flash-attention', 'grouped-query-attention'],
    learningObjectives: [
      'Explain the mismatch between speculative verifier batches and query-specific dynamic sparse layouts',
      'Trace how SpecAttn uses full-attention verification logits to choose sparse KV entries for the next draft',
      'Compare SpecSA naive sparse verification, exact merged scheduling, shared-index approximation, and refresh/reuse layers',
      'Use accepted-token throughput to reason about draft length, sparse ratio, KV traffic, and selection overhead',
    ],
    commonMisconception: 'Combining speculative decoding with sparse attention is not automatically multiplicative; sparse layouts must preserve reuse across verifier queries.',
  },
  'turboquant': {
    difficulty: 'advanced',
    estimatedMinutes: 35,
    prerequisites: ['kv-cache', 'grouped-query-attention', 'flash-attention', 'spec-sparse-attention'],
    learningObjectives: [
      'Compute how KV-cache memory scales with layers, context length, KV heads, head dimension, and precision',
      'Explain why preserving query-key dot products is different from minimizing vector reconstruction MSE',
      'Trace the TurboQuant pipeline from random rotation to MSE quantization to QJL residual correction',
      'Reason about low-bit KV compression tradeoffs using effective bits, dot-product bias, attention ranking, and outlier channels',
    ],
    commonMisconception: 'KV-cache quantization is not just rounding cached values; key compression must preserve the inner products that form attention logits.',
  },
  'kv-cache': {
    difficulty: 'advanced',
    estimatedMinutes: 18,
...
const CANONICAL_CATEGORY_BY_ANIMATION_ID = {
  ...
  'dapo-reasoning-rl': 'reinforcement-learning',
