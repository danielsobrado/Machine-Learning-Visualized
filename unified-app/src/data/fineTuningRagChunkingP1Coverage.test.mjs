...
import {
  FINE_TUNING_RAG_CHUNKING_P1_AUDITED_LESSON_IDS,
  FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS,
} from './fineTuningRagChunkingP1Coverage.js';
...
test('fine-tuning and RAG chunking P1 lessons remain priority curated assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of FINE_TUNING_RAG_CHUNKING_P1_AUDITED_LESSON_IDS) {
...
test('fine-tuning and RAG chunking P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('fine-tuning and RAG chunking competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS) {
...
test('fine-tuning and RAG chunking competencies require applied reasoning', () => {
  for (const requirement of FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS) {
...
test('fine-tuning and RAG chunking scenario evidence remains tagged P1', () => {
...
test('fine-tuning and RAG chunking contract keeps evidence independent', () => {
  const requirementIds = FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    FINE_TUNING_RAG_CHUNKING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...FINE_TUNING_RAG_CHUNKING_P1_AUDITED_LESSON_IDS].sort(),
