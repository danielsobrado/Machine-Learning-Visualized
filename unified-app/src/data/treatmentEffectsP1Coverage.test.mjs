...
import {
  TREATMENT_EFFECTS_P1_AUDITED_LESSON_IDS,
  TREATMENT_EFFECTS_P1_REQUIREMENTS,
} from './treatmentEffectsP1Coverage.js';
...
test('treatment effects remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of TREATMENT_EFFECTS_P1_AUDITED_LESSON_IDS) {
...
test('treatment effects P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    TREATMENT_EFFECTS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('treatment effects P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of TREATMENT_EFFECTS_P1_REQUIREMENTS) {
...
test('treatment effects P1 competencies require applied reasoning', () => {
  for (const requirement of TREATMENT_EFFECTS_P1_REQUIREMENTS) {
...
test('treatment effects scenario evidence remains tagged P1', () => {
...
test('treatment effects P1 contract keeps evidence independent', () => {
  const requirementIds = TREATMENT_EFFECTS_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    TREATMENT_EFFECTS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...TREATMENT_EFFECTS_P1_AUDITED_LESSON_IDS].sort(),
