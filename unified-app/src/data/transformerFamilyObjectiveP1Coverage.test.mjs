...
import {
  TRANSFORMER_FAMILY_OBJECTIVE_P1_AUDITED_LESSON_IDS,
  TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS,
} from './transformerFamilyObjectiveP1Coverage.js';
...
test('transformer family and objective P1 lessons remain priority curated assessments', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of TRANSFORMER_FAMILY_OBJECTIVE_P1_AUDITED_LESSON_IDS) {
...
test('transformer family and objective P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('transformer family and objective P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS) {
...
test('transformer family and objective P1 competencies require applied reasoning', () => {
  for (const requirement of TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS) {
...
test('transformer family and objective scenario evidence remains tagged P1', () => {
...
test('transformer family and objective P1 contract keeps ids unique and lessons aligned', () => {
  const requirementIds = TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    TRANSFORMER_FAMILY_OBJECTIVE_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...TRANSFORMER_FAMILY_OBJECTIVE_P1_AUDITED_LESSON_IDS].sort(),
