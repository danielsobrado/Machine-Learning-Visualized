...
import {
  ML_SECURITY_ROBUSTNESS_P1_AUDITED_LESSON_IDS,
  ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS,
} from './mlSecurityRobustnessP1Coverage.js';
...
test('ML security robustness remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);
  for (const lessonId of ML_SECURITY_ROBUSTNESS_P1_AUDITED_LESSON_IDS) {
...
test('ML security robustness P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('ML security robustness P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS) {
...
test('ML security robustness P1 competencies require applied reasoning', () => {
  for (const requirement of ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS) {
...
test('ML security robustness scenario evidence remains tagged P1', () => {
...
test('ML security robustness P1 contract keeps evidence independent', () => {
  const requirementIds = ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS.map(({ id }) => id);
  assert.equal(new Set(requirementIds).size, requirementIds.length, 'competency ids must be unique');
  const lessonIds = [...new Set(ML_SECURITY_ROBUSTNESS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId))].sort();
  assert.deepEqual(lessonIds, [...ML_SECURITY_ROBUSTNESS_P1_AUDITED_LESSON_IDS].sort());
