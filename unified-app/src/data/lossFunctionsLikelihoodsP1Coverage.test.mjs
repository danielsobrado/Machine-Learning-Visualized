...
import {
  LOSS_FUNCTIONS_LIKELIHOODS_P1_AUDITED_LESSON_IDS,
  LOSS_FUNCTIONS_LIKELIHOODS_P1_REQUIREMENTS,
} from './lossFunctionsLikelihoodsP1Coverage.js';
...
test('loss functions likelihoods P1 contract keeps evidence independent', () => {
  ...
  const requirementLessonIds = [...new Set(
    LOSS_FUNCTIONS_LIKELIHOODS_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...LOSS_FUNCTIONS_LIKELIHOODS_P1_AUDITED_LESSON_IDS].sort(),
