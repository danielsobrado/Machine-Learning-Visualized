...
import {
  SEQUENTIAL_TESTING_PEEKING_P1_AUDITED_LESSON_IDS,
  SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS,
} from './sequentialTestingPeekingP1Coverage.js';
...
test('sequential testing peeking remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of SEQUENTIAL_TESTING_PEEKING_P1_AUDITED_LESSON_IDS) {
...
test('sequential testing peeking P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('sequential testing peeking P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS) {
...
test('sequential testing peeking P1 competencies require applied reasoning', () => {
  for (const requirement of SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS) {
...
test('sequential testing peeking scenario evidence remains tagged P1', () => {
...
test('sequential testing peeking P1 contract keeps evidence independent', () => {
  const requirementIds = SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    SEQUENTIAL_TESTING_PEEKING_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...SEQUENTIAL_TESTING_PEEKING_P1_AUDITED_LESSON_IDS].sort(),
