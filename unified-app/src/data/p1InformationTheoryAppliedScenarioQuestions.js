  ...
  'cross-entropy': [
    {
      ...
      scenario: 'Two models classify the same example correctly, but Model A assigns the true class probability 0.8 while Model B assigns it 0.2 before the final argmax is compared with other classes. For a one-hot target, cross-entropy for this example is simply -ln(p_true). Use -ln(0.8) about 0.223 and -ln(0.2) about 1.609.',
      prompt: 'What does this example show about cross-entropy compared with a 0/1 correctness score?',
      choices: [
        'Model A receives loss about 0.223 while Model B receives about 1.609, so cross-entropy rewards assigning more probability to the true class even when a coarse correctness indicator cannot express that confidence difference',
