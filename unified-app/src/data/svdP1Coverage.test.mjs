...
import {
  SVD_P1_AUDITED_LESSON_IDS,
  SVD_P1_REQUIREMENTS,
} from './svdP1Coverage.js';
...
test('SVD remains a priority curated P1 assessment', () => {
  const priorityIds = new Set(PRIORITY_ASSESSMENT_LESSON_IDS);

  for (const lessonId of SVD_P1_AUDITED_LESSON_IDS) {
...
test('SVD P1 contract keeps required competencies explicit', () => {
  assert.deepEqual(
    SVD_P1_REQUIREMENTS.map(({ id }) => id).sort(),
    [...REQUIRED_COMPETENCY_IDS].sort(),
  );
});

test('SVD P1 competencies resolve to live assessment evidence', async (t) => {
  for (const requirement of SVD_P1_REQUIREMENTS) {
...
test('SVD P1 competencies require applied reasoning', () => {
  for (const requirement of SVD_P1_REQUIREMENTS) {
...
test('SVD scenario evidence remains tagged P1', () => {
...
test('SVD P1 contract keeps evidence independent', () => {
  const requirementIds = SVD_P1_REQUIREMENTS.map(({ id }) => id);
  ...
  const requirementLessonIds = [...new Set(
    SVD_P1_REQUIREMENTS.map(({ lessonId }) => lessonId),
  )].sort();
  assert.deepEqual(
    requirementLessonIds,
    [...SVD_P1_AUDITED_LESSON_IDS].sort(),
