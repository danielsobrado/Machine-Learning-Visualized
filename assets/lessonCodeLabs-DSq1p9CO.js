import{a as p}from"./assessment-data-BEcQsvEZ.js";const x=77,v=new Set(["and","for","the","as","of","to","in","vs","with","from","into","overview","track","comprehensive"]),y=new Map(p.map((e,t)=>[e.id,t]));function h(e){return[...new Set(e)]}function u(e){return h(String(e).toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>1&&!v.has(t)))}function k(e){return String(e).split(/[^a-zA-Z0-9]+/).filter(Boolean).map(n=>`${n[0].toUpperCase()}${n.slice(1)}`).join("")||"Lesson"}function l(e,t){const r=e.toLowerCase();return t.filter(n=>r.includes(n)).length}function w(e){const t=h([...u(e.id),...u(e.name)]);return t.length>0?t.slice(0,3):["lesson"]}function d(e){return`/animation/${e.id}`}function O({lesson:e,stepLabel:t,suffix:r,keyword:n,domain:o}){const a=`has${r}Keyword`;return{id:`${e.id}-keyword-check`,group:e.name,stepLabel:t,title:"Recognize the lesson keyword",concept:`${e.name} can be indexed by a stable keyword before deeper ${o.kind} logic runs.`,objective:"Return true when text contains the lesson keyword, case-insensitively.",difficulty:"warmup",starterCode:`function ${a}(text) {
...
check('lesson reference matches', ${a}(${JSON.stringify(`${e.name} ${d(e)}`)}), true);
check('lesson route matches', ${a}(${JSON.stringify(d(e))}), true);
...
}`,explanation:`Stable keywords help route learners and examples to the right ${e.name} code path.`}}function C({lesson:e,stepLabel:t,suffix:r,terms:n,domain:o}){const a=`count${r}FocusTerms`,s=l(e.name,n),c=`${e.name} ${e.description}`,b=l(c,n);return{id:`${e.id}-focus-term-count`,group:e.name,stepLabel:t,title:"Count focus terms",concept:`${o.kind} systems often reduce text into small signals before ranking or checking.`,objective:"Count how many lesson focus terms appear in the text.",difficulty:"core",starterCode:`function ${a}(text) {
...
check('title terms', ${a}(${JSON.stringify(e.name)}), ${s});
check('description terms', ${a}(${JSON.stringify(c)}), ${b});
...
return results;`,hints:["lower and terms are already prepared.","Use lower.includes(terms[i]) inside the loop.","if (lower.includes(terms[i])) count += 1;"],solution:`function ${a}(text) {
  ...
  return count;
}`,explanation:`This mirrors the small feature checks behind search, routing, and lesson-specific ${o.signalName} logic.`}}function A({lesson:e,stepLabel:t,suffix:r,domain:n}){const o=`best${r}Candidate`;return{id:`${e.id}-best-candidate`,group:e.name,stepLabel:t,title:"Select the best candidate",concept:`${n.kind} workflows often rank candidates by a score before choosing the next action.`,objective:"Return the id of the candidate with the highest score.",difficulty:"core",starterCode:`function ${o}(candidates) {
...
check('all stages present', ${o}(${JSON.stringify(a)}), true);
check('order does not matter', ${o}(${JSON.stringify(c)}), true);
check('missing one stage', ${o}(${JSON.stringify(s)}), false);
...
return results;`,hints:["Use stages.includes(requiredStages[i]).","Return false as soon as one required stage is absent.","if (!stages.includes(requiredStages[i])) return false;"],solution:`function ${o}(stages) {
  ...
  return true;
}`,explanation:`${n.stageExplanation} This check makes that dependency visible in code.`}}function T(e,t){const r=y.get(e.id),n=x+r,o=w(e),a=k(e.id);return{lessonId:e.id,lessonName:e.name,categoryId:e.categoryId,categoryName:e.categoryName,groupNumber:n,exercises:[O({lesson:e,stepLabel:`${n}.1`,suffix:a,keyword:o[0],domain:t}),C({lesson:e,stepLabel:`${n}.2`,suffix:a,terms:o,domain:t}),A({lesson:e,stepLabel:`${n}.3`,suffix:a,domain:t}),S({lesson:e,stepLabel:`${n}.4`,suffix:a,domain:t})]}}function j(e,t){return p.filter(r=>r.categoryId===e).map(r=>T(r,t))}function m(e,t,r){return e.map(n=>n.lessonId!==t?n:{...n,exercises:r(n).map((o,a)=>{var s;return{...o,group:n.lessonName,stepLabel:((s=n.exercises[a])==null?void 0:s.stepLabel)||`${n.groupNumber}.${a+1}`}})})}const M=[{id:"dot-product-first-pair",stepLabel:"1.1",group:"Dot product",title:"First matching pair",concept:"A dot product starts by multiplying entries with the same index. The first contribution comes from multiplying the two index-0 entries.",objective:"Replace one expression with the current aligned pair product inside the full matmul function.",difficulty:"warmup",starterCode:`/**
...
}`,explanation:"This is the dense-layer weight-gradient formula used in neural-network training."}],q=[{id:"transformer-token-embedding-lookup",stepLabel:"41.1",group:"Transformer mini-block shapes",title:"Token embedding lookup",concept:"A token ID selects one row from the embedding table.",objective:"Return embeddingTable[tokenId].",difficulty:"warmup",starterCode:`function lookupEmbedding(embeddingTable, tokenId) {
...
}`,explanation:"Many transformer bugs are shape bugs: Q and K must line up for similarity scores."}],N=[{id:"lm-vocab-size",stepLabel:"48.1",group:"Mini vocabulary and logits",title:"Vocabulary size",concept:"A language model predicts one score per vocabulary token.",objective:"Return the number of tokens in the vocabulary.",difficulty:"warmup",starterCode:`function vocabSize(vocab) {
...
}`,explanation:"Bias correction keeps Adam from underestimating early moments, while the second moment still rescales coordinates with different gradient magnitudes."}]),$=i("advanced-models",{kind:"advanced-model pipeline",signalName:"retrieval or multimodal score",stages:["encode","retrieve","ground"],stageExplanation:"Advanced model systems often encode inputs, retrieve or combine evidence, and check grounding."}),K=i("math-fundamentals",{kind:"mathematical computation",signalName:"numeric fit or stability score",stages:["represent","compute","check"],stageExplanation:"Math code is clearer when representation, computation, and result checks are separate."}),Q=i("core-ml",{kind:"machine-learning workflow",signalName:"validation metric",stages:["split","train","evaluate"],stageExplanation:"Core ML workflows depend on clean data splits, training logic, and honest evaluation."}),Y=i("model-reliability",{kind:"reliability check",signalName:"monitoring or risk score",stages:["observe","alert","triage"],stageExplanation:"Reliability systems observe behavior, alert on meaningful shifts, and triage failures."}),H=i("experimentation-causal-ml",{kind:"experiment analysis",signalName:"effect or balance score",stages:["assign","measure","compare"],stageExplanation:"Experiment code must separate assignment, measurement, and comparison to support causal claims."}),Z=i("probability-stats",{kind:"probability/statistics calculation",signalName:"probability or uncertainty score",stages:["count","normalize","summarize"],stageExplanation:"Probability code often counts outcomes, normalizes them, then summarizes uncertainty."}),ee=i("reinforcement-learning",{kind:"reinforcement-learning loop",signalName:"return or action-value score",stages:["observe","act","learn"],stageExplanation:"RL loops observe state, choose actions, and update behavior from feedback."}),te=m(ee,"ppo-clipped-policy-gradient",()=>[{id:"ppo-policy-ratio",title:"Compute the policy ratio",concept:"PPO compares the new policy probability with the old collection-policy probability for the sampled action.",objective:"Return pi_new divided by pi_old.",difficulty:"core",starterCode:`function policyRatio(newProbability, oldProbability) {
...
return results;`,hints:["PPO selects Math.min(unclipped, clipped).","A row is clipped when the selected value equals clipped and differs from unclipped.","if (Math.min(unclipped, clipped) !== unclipped) clippedCount += 1;"],solution:`function countClippedRows(rows, epsilon) {
  ...
  return clippedCount;
}`,explanation:"The minibatch audit links the PPO formula to the lesson table: not every out-of-band ratio clips, because the advantage sign decides which side is conservative."}]),ne=i("algorithms",{kind:"algorithmic data structure",signalName:"rank or membership score",stages:["insert","query","verify"],stageExplanation:"Algorithmic structures are useful when updates, queries, and checks are kept explicit."}),re=i("diffusion-models",{kind:"diffusion-model pipeline",signalName:"noise or denoising score",stages:["noise","condition","denoise"],stageExplanation:"Diffusion systems manage noise, conditioning, and denoising as separate implementation stages."}),g=[...U,...G,...F,...W,..._,...$,...K,...Q,...Y,...H,...Z,...te,...ne,...re],ae=Object.fromEntries(g.map(e=>[e.lessonId,e]));g.flatMap(e=>e.exercises);function oe(e){return ae[e]||null}function se(e){var t;return((t=oe(e))==null?void 0:t.exercises)||[]}export{g as L,se as g};
