import{a as p}from"./assessment-data-BEcQsvEZ.js";const x=77,k=new Set(["and","for","the","as","of","to","in","vs","with","from","into","overview","track","comprehensive"]),v=new Map(p.map((e,t)=>[e.id,t]));function h(e){return[...new Set(e)]}function l(e){return h(String(e).toLowerCase().split(/[^a-z0-9]+/).filter(t=>t.length>1&&!k.has(t)))}function y(e){return String(e).split(/[^a-zA-Z0-9]+/).filter(Boolean).map(a=>`${a[0].toUpperCase()}${a.slice(1)}`).join("")||"Lesson"}function u(e,t){const n=e.toLowerCase();return t.filter(a=>n.includes(a)).length}function w(e){const t=h([...l(e.id),...l(e.name)]);return t.length>0?t.slice(0,3):["lesson"]}function d(e){return`/animation/${e.id}`}function S({lesson:e,stepLabel:t,suffix:n,keyword:a,domain:o}){const r=`has${n}Keyword`;return{id:`${e.id}-keyword-check`,group:e.name,stepLabel:t,title:"Recognize the lesson keyword",concept:`${e.name} can be indexed by a stable keyword before deeper ${o.kind} logic runs.`,objective:"Return true when text contains the lesson keyword, case-insensitively.",difficulty:"warmup",starterCode:`function ${r}(text) {
...
return results;`,hints:["Multiply patch[r][c] by kernel[r][c].","Add it to sum.","sum += patch[r][c] * kernel[r][c];"],solution:`function conv2dPatchDot(patch, kernel) {
  ...
  return sum;
}`,explanation:"2D sliding window dot products extract translation-invariant spatial features from inputs."},{id:"max-pooling-2d-top",stepLabel:"33.1",group:"Window max",title:"Max pooling top row",concept:"Max pooling downsamples representations by selecting the maximum value in local sliding windows (e.g. 2x2). First, let's look at the top row.",objective:"Return the maximum value in the top row of a 2x2 patch.",difficulty:"warmup",starterCode:`function getPatchMax(patch2x2) {
...
return results;`,hints:["Use Math.max(patch2x2[0][0], patch2x2[0][1])."],solution:`function getPatchMax(patch2x2) {
  return Math.max(patch2x2[0][0], patch2x2[0][1]);
}`,explanation:"Max pooling extracts the strongest activation from each window."},{id:"max-pooling-2d-bottom",stepLabel:"33.2",group:"Window max",title:"Max pooling bottom row",concept:"Next, we look at the bottom row of the 2x2 patch.",objective:"Return the maximum value in the bottom row.",difficulty:"warmup",starterCode:`function getPatchMax(patch2x2) {
...
return results;`,hints:["Use Math.max(patch2x2[1][0], patch2x2[1][1])."],solution:`function getPatchMax(patch2x2) {
  const topMax = Math.max(patch2x2[0][0], patch2x2[0][1]);
  return Math.max(patch2x2[1][0], patch2x2[1][1]);
}`,explanation:"We now have the local maximums for both rows."},{id:"max-pooling-2d-global",stepLabel:"33.3",group:"Window max",title:"Global patch max",concept:"Finally, we compare the row maximums to find the global maximum for the patch.",objective:"Compare topMax and bottomMax and return the largest.",difficulty:"warmup",starterCode:`function getPatchMax(patch2x2) {
  const topMax = Math.max(patch2x2[0][0], patch2x2[0][1]);
...
return results;`,hints:["Use Math.max(topMax, bottomMax)."],solution:`function getPatchMax(patch2x2) {
  const topMax = Math.max(patch2x2[0][0], patch2x2[0][1]);
  const bottomMax = Math.max(patch2x2[1][0], patch2x2[1][1]);
  
  return Math.max(topMax, bottomMax);
}`,explanation:"The global patch maximum is the final output of the 2x2 max pooling operation."},{id:"max-pooling-2d-optimized",stepLabel:"33.4",group:"Window max",title:"Optimized Max pooling",concept:"We can write this in a single optimized Math.max call.",objective:"Condense the operations into a single Math.max.",difficulty:"core",starterCode:`function getPatchMax(patch2x2) {
...
return results;`,hints:["Use Math.max(patch2x2[0][0], patch2x2[0][1], patch2x2[1][0], patch2x2[1][1])."],solution:`function getPatchMax(patch2x2) {
  return Math.max(patch2x2[0][0], patch2x2[0][1], patch2x2[1][0], patch2x2[1][1]);
}`,explanation:"Max pooling extracts the strongest activation from each window, ensuring model robustness to translation shifts."},{id:"conv-relu-identity",stepLabel:"34.1",group:"ReLU clip",title:"Linear Identity Pass",concept:"Before adding non-linearity, we begin with a simple identity pass that returns the input unchanged.",objective:"Return the input x unmodified.",difficulty:"warmup",starterCode:`function relu(x) {
  // TODO: return x
  return 0;
}`,testCode:`const results = [];
function check(name, actual, expected) {
...
return results;`,hints:["Return x."],solution:`function relu(x) {
  return x;
}`,explanation:"Without non-linearity, deep networks collapse into a single linear transformation."},{id:"conv-relu-conditional",stepLabel:"34.2",group:"ReLU clip",title:"Conditional Branch",concept:"The ReLU function breaks linearity by treating positive and negative values differently.",objective:"Add an if statement to return x if it is positive.",difficulty:"warmup",starterCode:`function relu(x) {
...
return results;`,hints:["if (x > 0) return x;"],solution:`function relu(x) {
  if (x > 0) {
    return x;
  }
  return 0;
}`,explanation:'The conditional creates a "hinge" at zero, which provides the non-linearity needed to learn complex functions.'},{id:"conv-relu-negative",stepLabel:"34.3",group:"ReLU clip",title:"Negative Clamping",concept:"If the value is not positive, ReLU clamps it to zero.",objective:"Ensure negative values return 0.",difficulty:"warmup",starterCode:`function relu(x) {
  if (x > 0) {
    return x;
  }
...
return results;`,hints:["Return 0 at the end of the function."],solution:`function relu(x) {
  if (x > 0) {
    return x;
  }
  return 0;
}`,explanation:"Zeroing out negative values adds sparsity to the network activations."},{id:"conv-relu-activation",stepLabel:"34.4",group:"ReLU clip",title:"Conv + ReLU activation",concept:"We can optimize this logic into a single line using the Math.max function.",objective:"Apply the ReLU activation function using Math.max(0, x).",difficulty:"core",starterCode:`function relu(x) {
...
return results;`,hints:["Use Math.max(0, x)."],solution:`function relu(x) {
  return Math.max(0, x);
}`,explanation:"ReLU introduces non-linear decision boundaries, enabling neural networks to model non-linear mappings."},{id:"init-he-inverse-fanin",stepLabel:"35.1",group:"He std",title:"Inverse Fan-In",concept:"He initialization scales random weights to prevent vanishing/exploding gradients. First, we compute the inverse fan-in.",objective:"Compute 1 / fanIn.",difficulty:"warmup",starterCode:`function getHeStd(fanIn) {
...
return results;`,hints:["Divide 1 by fanIn."],solution:`function getHeStd(fanIn) {
  const invFanIn = 1 / fanIn;
  
  return invFanIn;
}`,explanation:"As the number of inputs (fan-in) increases, the variance of the output grows. We divide by fan-in to keep the variance stable."},{id:"init-he-variance",stepLabel:"35.2",group:"He std",title:"ReLU Variance Scaling",concept:"Because ReLU throws away exactly half the activations (the negative ones), we must multiply the variance by 2 to compensate.",objective:"Multiply the inverse fan-in by 2 to get the He variance.",difficulty:"core",starterCode:`function getHeStd(fanIn) {
  const invFanIn = 1 / fanIn;
...
return results;`,hints:["Multiply invFanIn by 2."],solution:`function getHeStd(fanIn) {
  const invFanIn = 1 / fanIn;
  
  const variance = 2 * invFanIn;
  
  return variance;
}`,explanation:"Without this factor of 2, the variance of the activations would halve at every layer, leading to vanishing gradients."},{id:"init-he-std-calc",stepLabel:"35.3",group:"He std",title:"Standard Deviation Extraction",concept:"Initialization functions need the standard deviation, which is the square root of the variance.",objective:"Compute the standard deviation from the variance.",difficulty:"warmup",starterCode:`function getHeStd(fanIn) {
  const invFanIn = 1 / fanIn;
  const variance = 2 * invFanIn;
...
return results;`,hints:["Use Math.sqrt(variance)."],solution:`function getHeStd(fanIn) {
  const invFanIn = 1 / fanIn;
  const variance = 2 * invFanIn;
  ...
  return std;
}`,explanation:"The standard deviation defines the spread of the Gaussian distribution from which we will sample the initial weights."},{id:"init-he-std",stepLabel:"35.4",group:"He std",title:"He initialization standard deviation",concept:"We can write this cleanly in a single mathematical formula: Math.sqrt(2 / fanIn).",objective:"Return the He standard deviation directly.",difficulty:"core",starterCode:`function getHeStd(fanIn) {
...
}`,explanation:"Scaling weights by He standard deviation maintains constant activation variance across layers."},{id:"vae-reparameterize",stepLabel:"VAE.1",group:"Reparameterize z",title:"Reparameterization Trick",concept:"The reparameterization trick allows gradients to backpropagate through stochastic nodes by shifting the randomness to an auxiliary variable eps.",objective:"Compute the sampled latent z: z = mu + exp(0.5 * logvar) * eps.",difficulty:"warmup",starterCode:`/**
...
return results;`,hints:["Compute std using Math.exp(0.5 * logvar).","Compute z using mu + std * eps."],solution:`function vaeStep(x, xHat, mu, logvar, eps) {
  ...
  const elbo = 0;
  return { z, reconLoss, klLoss, elbo };
}`,explanation:"By separating the deterministic mean/variance from the random noise (eps), the network remains fully differentiable."},{id:"vae-recon-mse",stepLabel:"VAE.2",group:"Reconstruction MSE",title:"Reconstruction Error",concept:"The reconstruction loss forces the decoder to accurately recreate the original input from the latent z. We use Mean Squared Error (MSE).",objective:"Compute the scalar reconstruction loss: (x - xHat)^2.",difficulty:"warmup",starterCode:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
...
return results;`,hints:["Compute the difference x - xHat, then square it.","Math.pow(x - xHat, 2) or (x - xHat) * (x - xHat)."],solution:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  ...
  const elbo = 0;
  return { z, reconLoss, klLoss, elbo };
}`,explanation:"Minimizing reconstruction loss ensures the latent space captures all necessary information to rebuild the input."},{id:"vae-kl-divergence",stepLabel:"VAE.3",group:"KL closed form",title:"KL Divergence",concept:"The KL divergence regularizer forces the latent distribution to match a standard Normal distribution (mean 0, variance 1).",objective:"Compute the KL loss per dim: -0.5 * (1 + logvar - mu^2 - exp(logvar)).",difficulty:"core",starterCode:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
...
return results;`,hints:["Compute mu * mu and Math.exp(logvar).","Multiply the entire sum by -0.5."],solution:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
  ...
  const elbo = 0;
  return { z, reconLoss, klLoss, elbo };
}`,explanation:"This closed-form solution only works because we assume the prior is a standard Gaussian."},{id:"vae-elbo",stepLabel:"VAE.4",group:"Combined ELBO",title:"Evidence Lower Bound (ELBO)",concept:"The total VAE loss is derived from the Evidence Lower Bound. The loss to minimize is the sum of reconstruction error and KL divergence.",objective:"Compute the final loss (negative ELBO): reconLoss + klLoss.",difficulty:"warmup",starterCode:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
  const klLoss = -0.5 * (1 + logvar - mu * mu - Math.exp(logvar));
...
return results;`,hints:["Add reconLoss and klLoss."],solution:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
  const klLoss = -0.5 * (1 + logvar - mu * mu - Math.exp(logvar));
  ...
  return { z, reconLoss, klLoss, elbo };
}`,explanation:"By minimizing this joint loss, the VAE balances generating accurate reconstructions with maintaining a smoothly organized latent space."},{id:"vae-edge-cases",stepLabel:"VAE.5",group:"Latent edge cases",title:"Standard Normal Edge Case",concept:"When the encoder perfectly matches the prior (mu=0, logvar=0), the KL divergence drops exactly to 0, and z becomes exactly the sampled noise eps.",objective:"Set matchesStandardPrior when mu and logvar are both 0 and klLoss is 0.",difficulty:"warmup",starterCode:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
  const klLoss = -0.5 * (1 + logvar - mu * mu - Math.exp(logvar));
...
return results;`,hints:["All three conditions must hold: mu === 0, logvar === 0, klLoss === 0.","matchesStandardPrior = mu === 0 && logvar === 0 && klLoss === 0;"],solution:`function vaeStep(x, xHat, mu, logvar, eps) {
  const std = Math.exp(0.5 * logvar);
  const z = mu + std * eps;
  const reconLoss = (x - xHat) * (x - xHat);
  const klLoss = -0.5 * (1 + logvar - mu * mu - Math.exp(logvar));
  ...
  return { z, reconLoss, klLoss, elbo, matchesStandardPrior };
}`,explanation:"This proves that a standard Normal distribution incurs zero penalty from the KL divergence term."},{id:"multimodal-llm-dot",stepLabel:"37.1",group:"Linear project",title:"Modality Dot Product",concept:"Multimodal LLMs project visual token embeddings into the text vocabulary hidden space using a learned projection matrix. First, we compute the dot product between the input patch and a single row of the projection matrix.",objective:"Compute the dot product of patch and projector[j].",difficulty:"warmup",starterCode:`function projectPatch(patch, projector, outDim) {
...
return results;`,hints:["sum += patch[d] * projector[d][j];"],solution:`function projectPatch(patch, projector, outDim) {
  const projected = Array(outDim).fill(0);
  ...
  return projected;
}`,explanation:"A dot product measures the similarity and maps the spatial patch features into the text embedding space."},{id:"multimodal-llm-loop",stepLabel:"37.2",group:"Linear project",title:"Matrix Vector Loop",concept:"We iterate over all output dimensions to perform a full matrix-vector multiplication.",objective:"Wrap the dot product logic in a loop over j from 0 to outDim.",difficulty:"core",starterCode:`function projectPatch(patch, projector, outDim) {
  const projected = Array(outDim).fill(0);
...
return results;`,hints:["Add a for loop for j.","for (let j = 0; j < outDim; j++) { ... }"],solution:`function projectPatch(patch, projector, outDim) {
  const projected = Array(outDim).fill(0);
  ...
  return projected;
}`,explanation:"This operation acts as a translator, taking raw visual features and converting them into a language the text model understands."},{id:"multimodal-llm-project",stepLabel:"37.3",group:"Linear project",title:"Multimodal token projection",concept:"The full linear projection is the core bridging mechanism connecting distinct modality encoders to a single unified LLM backbone.",objective:"Finalize and return the projected token.",difficulty:"challenge",starterCode:`function projectPatch(patch, projector, outDim) {
  const projected = Array(outDim).fill(0);
  for (let j = 0; j < outDim; j++) {
    let sum = 0;
    for (let d = 0; d < patch.length; d++) {
...
check('project 2D to 3D', projectPatch([1, 2], [[0.5, 0, 1], [0, 0.5, 2]], 3), [0.5, 1, 5]);
return results;`,hints:["Return projected."],solution:`function projectPatch(patch, projector, outDim) {
  const projected = Array(outDim).fill(0);
  for (let j = 0; j < outDim; j++) {
    let sum = 0;
    for (let d = 0; d < patch.length; d++) {
  ...
  return projected;
}`,explanation:"By converting images into text-like embeddings, multimodal models can smoothly alternate between seeing and speaking."}],q=[{id:"transformer-token-embedding-lookup",stepLabel:"41.1",group:"Transformer mini-block shapes",title:"Token embedding lookup",concept:"A token ID selects one row from the embedding table.",objective:"Return embeddingTable[tokenId].",difficulty:"warmup",starterCode:`function lookupEmbedding(embeddingTable, tokenId) {
...
}`,explanation:"Hinge loss penalizes the model when the positive item score is not higher than the negative item score by at least the margin."},{id:"eval-pass-at-k-edge",stepLabel:"28.1",group:"Pass@k",title:"Guaranteed Pass Condition",concept:"Pass@k measures LLM code generation quality: probability that at least one of k generated samples passes tests. If the number of failing samples (n - c) is less than k, you are guaranteed to pick at least one passing sample.",objective:"If n - c < k, return 1.0.",difficulty:"warmup",starterCode:`function passAtK(n, c, k) {
...
return results;`,hints:["if (n - c < k) return 1.0;"],solution:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  ...
  return 1.0 - prod;
}`,explanation:"By checking this early, we avoid calculating probabilities for impossible failing combinations."},{id:"eval-pass-at-k-base",stepLabel:"28.2",group:"Pass@k",title:"Failure Probability Initialization",concept:"To find the probability of at least one pass, we first calculate the probability that ALL k samples fail. We initialize the product to 1.0.",objective:"Initialize a variable prod to 1.0.",difficulty:"warmup",starterCode:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
...
return results;`,hints:["let prod = 1.0;"],solution:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
    prod *= (n - c - i) / (n - i);
  ...
  return 1.0 - prod;
}`,explanation:"A product accumulator must start at 1.0 to multiply consecutive probabilities."},{id:"eval-pass-at-k-loop",stepLabel:"28.3",group:"Pass@k",title:"Sampling Iteration",concept:"We iterate k times, representing drawing k samples without replacement from the n total samples.",objective:"Create a loop from i = 0 to i < k.",difficulty:"warmup",starterCode:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
...
return results;`,hints:["for (let i = 0; i < k; i++) {","  prod *= (n - c - i) / (n - i);","}"],solution:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
    prod *= (n - c - i) / (n - i);
  ...
  return 1.0 - prod;
}`,explanation:"This models sampling k times in a row."},{id:"eval-pass-at-k-accumulate",stepLabel:"28.4",group:"Pass@k",title:"Failure Chain Accumulation",concept:"At step i, there are (n - i) remaining samples to pick from, and (n - c - i) of them are failing samples. We multiply the current failure probability by this ratio.",objective:"Multiply prod by (n - c - i) / (n - i).",difficulty:"core",starterCode:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
...
check('pass@2 with c=2', passAtK(10, 2, 2), 0.377778);
return results;`,hints:["prod *= (n - c - i) / (n - i);"],solution:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
    prod *= (n - c - i) / (n - i);
  ...
  return 1.0 - prod;
}`,explanation:"The product chain computes the total probability that every single sampled output is a failure."},{id:"eval-pass-at-k",stepLabel:"28.5",group:"Pass@k",title:"Pass@k Final Metric",concept:"Since we computed the probability that ALL k samples fail, the probability that AT LEAST ONE sample passes is its complement: 1.0 - prod.",objective:"Return 1.0 - prod.",difficulty:"core",starterCode:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
    prod *= (n - c - i) / (n - i);
...
check('pass@2 with c=2', passAtK(10, 2, 2), 0.377778);
return results;`,hints:["return 1.0 - prod;"],solution:`function passAtK(n, c, k) {
  if (n - c < k) return 1.0;
  let prod = 1.0;
  for (let i = 0; i < k; i++) {
    prod *= (n - c - i) / (n - i);
...
return results;`,hints:["Use adjustmentSet.includes(confounders[i]).","If one is missing, return false.","if (!adjustmentSet.includes(confounders[i])) return false;"],solution:`function coversConfounders(adjustmentSet, confounders) {
  ...
  return true;
}`,explanation:"This toy check is not full d-separation, but it reinforces the adjustment-set idea."}],E=[{id:"word2vec-dot-similarity",stepLabel:"1.1",group:"Similarity score",title:"Vector Similarity Score",concept:"Skip-gram negative sampling compares center and context embeddings with a dot product. That score drives both the positive likelihood and every negative sample term.",objective:"Inside skipGramTrainStep, compute the positive dot product posScore between vCenter and vContext.",difficulty:"warmup",starterCode:`/**
...
function ddpmSamplingStep(xt, epsTheta, t, totalSteps, betaStart, betaEnd, zNoise, x0, forwardNoise) {
  ...
  return { betas, alphaBars, forwardXt, reverseXt };
}`,explanation:"Stochastic reverse steps maintain sample diversity while the t = 0 step returns a deterministic denoised value."},{id:"cfg-uncond-branch",stepLabel:"CFG.1",group:"Uncond branch",title:"Unconditional Generation",concept:"Classifier-Free Guidance blends a conditional and an unconditional prediction. If the scale is 0, we simply return the unconditional prediction.",objective:"Implement the unconditional branch when scale is 0.",difficulty:"warmup",starterCode:`/**
...
return results;`,hints:["Check if scale === 0.","Return { guided: epsUncond, delta: 0 }."],solution:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
  if (scale === 0) {
  ...
  const guided = epsUncond + scale * delta;
  return { guided, delta };
}`,explanation:"A scale of 0 completely ignores the text prompt, generating an unguided image."},{id:"cfg-cond-branch",stepLabel:"CFG.2",group:"Cond branch",title:"Standard Generation",concept:"If the scale is exactly 1, we don't apply any extra guidance amplification. We simply use the conditional prediction.",objective:"Implement the conditional branch when scale is 1.",difficulty:"warmup",starterCode:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
  if (scale === 0) {
    return { guided: epsUncond, delta: 0 };
  }
...
return results;`,hints:["Check if scale === 1.","Return { guided: epsCond, delta: epsCond - epsUncond }."],solution:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
  if (scale === 0) {
    return { guided: epsUncond, delta: 0 };
  }
  ...
  const guided = epsUncond + scale * delta;
  return { guided, delta };
}`,explanation:"A scale of 1 means standard diffusion generation without over-amplifying the text conditioning."},{id:"cfg-scale-mix",stepLabel:"CFG.3",group:"Scale mix",title:"Guidance Amplification",concept:"The true power of CFG comes from setting the scale > 1. We calculate the direction pointing from unconditional to conditional, and amplify it.",objective:"Compute the guided prediction using the CFG formula: epsUncond + scale * (epsCond - epsUncond).",difficulty:"core",starterCode:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
...
return results;`,hints:["Compute delta = epsCond - epsUncond.","Compute guided = epsUncond + scale * delta."],solution:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
  if (scale === 0) return { guided: epsUncond, delta: 0 };
  if (scale === 1) return { guided: epsCond, delta: epsCond - epsUncond };
  ...
  return { guided, delta };
}`,explanation:"By pushing the prediction further away from the unconditional baseline, the model generates images that are much more strongly aligned with the text prompt."},{id:"cfg-zero-identity",stepLabel:"CFG.4",group:"Zero-scale identity",title:"Mathematical Identity",concept:"The branches for scale 0 and scale 1 are actually mathematically redundant! If we just evaluate the core formula for scale 0, it gives the exact same result.",objective:"Verify that the core formula inherently handles the scale 0 and 1 cases.",difficulty:"warmup",starterCode:`function cfgGuidanceStep(epsCond, epsUncond, scale) {
...
return results;`,hints:["Top-left patch: row 0-1, col 0-1.","Top-right patch: row 0-1, col 2-3.","Bottom-left patch: row 2-3, col 0-1.","Bottom-right patch: row 2-3, col 2-3.","Store each group in a 4-element array and push to tokens."],solution:`function patchifyImage(image2D, patchSize) {
  ...
  return tokens;
}`,explanation:"Patchifying translates spatial grids into coordinate-grouped tokens compatible with sequence transformer heads."},{id:"sd3-latent-dims-w",stepLabel:"76.1",group:"VAE downscale",title:"SD3 Latent Width Calculation",concept:"Stable Diffusion 3 downsamples images by a factor of 8 during VAE encoding. First, we compute the latent width.",objective:"Divide the pixel width by the spatial compression factor (8).",difficulty:"warmup",starterCode:`function getSd3LatentShape(width, height) {
...
return results;`,hints:["Divide width by 8."],solution:`function getSd3LatentShape(width, height) {
  const wLat = width / 8;
  return [wLat, 0];
}`,explanation:"The latent dimension is significantly smaller, making it easier for the network to process spatial features."},{id:"sd3-latent-dims-h",stepLabel:"76.2",group:"VAE downscale",title:"SD3 Latent Height Calculation",concept:"Next, we compute the latent height using the same downscale factor of 8.",objective:"Divide the pixel height by 8.",difficulty:"warmup",starterCode:`function getSd3LatentShape(width, height) {
  const wLat = width / 8;
...
return results;`,hints:["Divide height by 8."],solution:`function getSd3LatentShape(width, height) {
  const wLat = width / 8;
  const hLat = height / 8;
  
  return [wLat, hLat];
}`,explanation:"Both width and height are compressed independently by the VAE."},{id:"sd3-latent-dims-array",stepLabel:"76.3",group:"VAE downscale",title:"Array Construction",concept:"We store the resulting downscaled dimensions in a shape array.",objective:"Construct the array holding the width and height.",difficulty:"warmup",starterCode:`function getSd3LatentShape(width, height) {
  const wLat = width / 8;
  const hLat = height / 8;
...
check('512x512 resolution', getSd3LatentShape(512, 512), [64, 64]);
return results;`,hints:["Use [wLat, hLat]."],solution:`function getSd3LatentShape(width, height) {
  const wLat = width / 8;
  const hLat = height / 8;
  ...
  return shape;
}`,explanation:"The shape array represents the spatial footprint of the latent representation."},{id:"sd3-latent-dims",stepLabel:"76.4",group:"VAE downscale",title:"SD3 Latent Shape Calculation",concept:"Stable Diffusion 3 downsamples images by a factor of 8 during VAE encoding: H_lat = H / 8, W_lat = W / 8.",objective:"Optimize the function to return the downscaled dimensions directly.",difficulty:"core",starterCode:`function getSd3LatentShape(width, height) {
...
}`,explanation:"Integrating velocity steps generates data along straight trajectories, yielding better samples with fewer steps."},{id:"vae-latent-scaling-factor",stepLabel:"78.1",group:"encode scale",title:"Scale Factor Definition",concept:"Latent vectors are scaled to ensure training stability and match unit-variance Gaussian distributions.",objective:"Define the standard SD scaling factor of 0.18215.",difficulty:"warmup",starterCode:`function scaleLatent(val) {
...
return results;`,hints:["Set factor to 0.18215."],solution:`function scaleLatent(val) {
  const factor = 0.18215;
  return val;
}`,explanation:"The factor is empirically determined during VAE pre-training to enforce unit variance across the latent space."},{id:"vae-latent-scaling-multiply",stepLabel:"78.2",group:"encode scale",title:"Latent Scaling Application",concept:"We multiply the raw latent values by the scaling factor.",objective:"Multiply the latent val by the scaling factor.",difficulty:"warmup",starterCode:`function scaleLatent(val) {
  const factor = 0.18215;
...
return results;`,hints:["scaled = val * factor;"],solution:`function scaleLatent(val) {
  const factor = 0.18215;
  
  const scaled = val * factor;
  
  return scaled;
}`,explanation:"Multiplying by the scale factor adjusts the standard deviation of the latents."},{id:"vae-latent-scaling-return",stepLabel:"78.3",group:"encode scale",title:"Optimized Latent Scaling",concept:"We can supply the factor as a default argument and perform the multiplication inline.",objective:"Return val multiplied by factor directly.",difficulty:"warmup",starterCode:`function scaleLatent(val, factor = 0.18215) {
...
check('standard scale', scaleLatent(5.0), 0.91075);
return results;`,hints:["return val * factor;"],solution:`function scaleLatent(val, factor = 0.18215) {
  return val * factor;
}`,explanation:"This clean abstraction allows us to scale latents easily without hardcoding the factor inside the logic."},{id:"vae-latent-scaling",stepLabel:"78.4",group:"encode scale",title:"VAE Latent Scaling",concept:"Scaling coordinates avoids vanishing activation magnitudes, preserving gradients across deep blocks.",objective:"Apply the standard scaling factor (e.g. 0.18215) to visual latent representation values.",difficulty:"core",starterCode:`function scaleLatent(val, factor = 0.18215) {
...
return results;`,hints:["Loop index i from 0 to wordTokens.length - 1.","If i < length - 1 and wordTokens[i] === pairTarget[0] and wordTokens[i+1] === pairTarget[1], push combined string and increment i.","Otherwise, push wordTokens[i]."],solution:`function mergePair(wordTokens, pairTarget) {
  ...
  return merged;
}`,explanation:"Replacing character pairs progressively builds longer subword vocabularies, reducing sequence token counts."},{id:"clip-l2-norm-sumsq",stepLabel:"80.1",group:"L2 normalize",title:"Sum of Squares",concept:"CLIP maps text and image embeddings to a shared latent space. To compute L2 normalization, we first find the sum of squared elements.",objective:"Accumulate the square of each element in the vector.",difficulty:"warmup",starterCode:`function l2Normalize(vec) {
  let sumSq = 0;
...
return results;`,hints:["sumSq += vec[i] * vec[i];"],solution:`function l2Normalize(vec) {
  let sumSq = 0;
  ...
  return vec;
}`,explanation:"Squaring the elements removes negative signs and emphasizes larger magnitudes."},{id:"clip-l2-norm-sqrt",stepLabel:"80.2",group:"L2 normalize",title:"L2 Vector Norm",concept:"The L2 norm (magnitude) is the square root of the sum of squares.",objective:"Compute the square root of sumSq.",difficulty:"warmup",starterCode:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
...
check('no op yet', l2Normalize([3.0, 4.0]), [3.0, 4.0]);
return results;`,hints:["const norm = Math.sqrt(sumSq);"],solution:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
  ...
  return vec;
}`,explanation:"This represents the geometric length of the vector in high-dimensional space."},{id:"clip-l2-norm-div",stepLabel:"80.3",group:"L2 normalize",title:"Vector Normalization",concept:"To project the vector onto a unit hypersphere, we divide each coordinate by the calculated norm.",objective:"Return a new array where each element is divided by norm.",difficulty:"warmup",starterCode:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
...
return results;`,hints:["return vec.map(x => x / norm);"],solution:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
...
}`,explanation:"Now the vector has a length of exactly 1.0, making dot products directly equivalent to cosine similarity."},{id:"clip-l2-norm",stepLabel:"80.4",group:"L2 normalize",title:"CLIP Vector L2 Normalization",concept:"We must also add a safeguard to avoid dividing by zero if an empty or zero vector is passed in.",objective:"Add a check to return vec directly if norm is 0.",difficulty:"core",starterCode:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
...
return results;`,hints:["if (norm === 0) return vec;"],solution:`function l2Normalize(vec) {
  let sumSq = 0;
  for (let i = 0; i < vec.length; i++) {
    sumSq += vec[i] * vec[i];
  }
  ...
  return vec.map(x => x / norm);
}`,explanation:"L2 normalization simplifies cosine similarity calculations to basic dot products, accelerating retrieval and matching loops."},{id:"t5-pad-attention-mask-init",stepLabel:"81.1",group:"pad mask",title:"Mask Array Initialization",concept:"T5 text encoders block attention to padding tokens by constructing boolean masks. We start with an empty array.",objective:"Create and return an empty mask array.",difficulty:"warmup",starterCode:`function getAttentionMask(tokenIds, padId) {
...
return results;`,hints:["return [];"],solution:`function getAttentionMask(tokenIds, padId) {
  const mask = [];
  return mask;
}`,explanation:"The attention mask has the same sequence length as the tokenized text input."},{id:"t5-pad-attention-mask-loop",stepLabel:"81.2",group:"pad mask",title:"Token Iteration",concept:"We iterate through each token in the token sequence.",objective:"Create a loop over the tokenIds and append 1 for each token.",difficulty:"warmup",starterCode:`function getAttentionMask(tokenIds, padId) {
  const mask = [];
...
return results;`,hints:["for (let i = 0; i < tokenIds.length; i++) { mask.push(1); }"],solution:`function getAttentionMask(tokenIds, padId) {
  const mask = [];
  for (let i = 0; i < tokenIds.length; i++) {
    mask.push(1);
  }
  return mask;
}`,explanation:"A 1 indicates that the attention mechanism should process this token."},{id:"t5-pad-attention-mask-cond",stepLabel:"81.3",group:"pad mask",title:"Padding Condition",concept:"If the token matches the padId, it should be masked out with a 0 instead of a 1.",objective:"Add a condition to push 0 if the token is padId.",difficulty:"core",starterCode:`function getAttentionMask(tokenIds, padId) {
  const mask = [];
  for (let i = 0; i < tokenIds.length; i++) {
...
return results;`,hints:["if (tokenIds[i] === padId) { mask.push(0); } else { mask.push(1); }"],solution:`function getAttentionMask(tokenIds, padId) {
  const mask = [];
  for (let i = 0; i < tokenIds.length; i++) {
    if (tokenIds[i] === padId) {
      mask.push(0);
  ...
  return mask;
}`,explanation:"This manual loop explicitly builds the binary padding mask."},{id:"t5-pad-attention-mask",stepLabel:"81.4",group:"pad mask",title:"T5 Padding Attention Mask",concept:"We can optimize this into a single mapping operation.",objective:"Generate a binary attention mask using tokenIds.map.",difficulty:"warmup",starterCode:`function getAttentionMask(tokenIds, padId) {
...
}`,explanation:"Padding masks prevent models from aggregating meaningless information from trailing fill tokens."},{id:"joint-attn-concat-init",stepLabel:"82.1",group:"Concat Q",title:"Array Expansion",concept:"SD3's Joint Attention block concatenates text and image tokens along the sequence dimension. We start by copying the text embeddings.",objective:"Create a new array containing all elements of textEmbeds.",difficulty:"warmup",starterCode:`function concatEmbeddings(textEmbeds, imageEmbeds) {
...
return results;`,hints:["Use the spread operator [...textEmbeds]."],solution:`function concatEmbeddings(textEmbeds, imageEmbeds) {
  const joint = [...textEmbeds];
  
  return joint;
}`,explanation:"We must not mutate the original sequence, so we create a new joint sequence array."},{id:"joint-attn-concat-loop",stepLabel:"82.2",group:"Concat Q",title:"Image Append Loop",concept:"Next, we append each image token sequentially to the end of the text tokens.",objective:"Loop through imageEmbeds and push each token to joint.",difficulty:"warmup",starterCode:`function concatEmbeddings(textEmbeds, imageEmbeds) {
  const joint = [...textEmbeds];
...
return results;`,hints:["for (let i = 0; i < imageEmbeds.length; i++) { joint.push(imageEmbeds[i]); }"],solution:`function concatEmbeddings(textEmbeds, imageEmbeds) {
  const joint = [...textEmbeds];
  ...
  return joint;
}`,explanation:"The resulting sequence length is seq_txt + seq_img."},{id:"joint-attn-concat-seq",stepLabel:"82.3",group:"Concat Q",title:"Multimodal Sequence Concatenation",concept:"We can optimize this significantly using native array concatenation methods.",objective:"Concatenate text and image token lists into a combined multimodal sequence in one line.",difficulty:"warmup",starterCode:`function concatEmbeddings(textEmbeds, imageEmbeds) {
...
}`,explanation:"Concatenating modalities enables bidirectional cross-attention without separate cross-attention layers."},{id:"dit-time-embed",stepLabel:"DiT.1",group:"Time embed inject",title:"Time Embed Injection",concept:"Diffusion Transformers (DiT) use a time-conditioned embedding (tEmb) to derive per-dimension scale and shift parameters for adaptive Layer Normalization (adaLN).",objective:"Accept precomputed scale and shift from the time embedding, and return an empty block state.",difficulty:"warmup",starterCode:`/**
...
return results;`,hints:["The arrays are already initialized. Just run the test."],solution:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = [];
  const afterAttn = [];
  const afterMlp = [];
  
  return { modulated, afterAttn, afterMlp };
}`,explanation:"In a real implementation, the time embedding is passed through a linear projection to derive these scale and shift vectors dynamically."},{id:"dit-adaln",stepLabel:"DiT.2",group:"adaLN scale/shift",title:"Adaptive LayerNorm",concept:"We apply the adaLN modulation by scaling and shifting the hidden state x using the time-derived parameters.",objective:"Compute the modulated state: x[i] * (1 + scale[i]) + shift[i].",difficulty:"core",starterCode:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
...
return results;`,hints:["Iterate over x using a for loop.","For each element, multiply x[i] by (1 + scale[i]) and add shift[i]."],solution:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = new Array(x.length).fill(0);
  ...
  return { modulated, afterAttn, afterMlp };
}`,explanation:"By shifting and scaling the activations, the time step conditions the network to denoise properly at that specific noise level."},{id:"dit-attn-residual",stepLabel:"DiT.3",group:"Self-attn residual",title:"Attention Residual Connection",concept:"After calculating self-attention (which we stub here with attnOut), we add a residual connection from the modulated state.",objective:"Compute the afterAttn state: modulated[i] + attnOut[i].",difficulty:"warmup",starterCode:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = new Array(x.length).fill(0);
  for (let i = 0; i < x.length; i++) {
    modulated[i] = x[i] * (1 + scale[i]) + shift[i];
  }
...
return results;`,hints:["Iterate over the elements.","Set afterAttn[i] to modulated[i] + attnOut[i]."],solution:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = new Array(x.length).fill(0);
  for (let i = 0; i < x.length; i++) {
    modulated[i] = x[i] * (1 + scale[i]) + shift[i];
  }
  ...
  return { modulated, afterAttn, afterMlp };
}`,explanation:"Residual connections ensure smooth gradient flow and allow the network to learn perturbations rather than entirely new states."},{id:"dit-mlp-residual",stepLabel:"DiT.4",group:"MLP residual",title:"MLP Residual Connection",concept:"The final step of the DiT block adds a residual connection around the Multi-Layer Perceptron (MLP) component.",objective:"Compute the afterMlp state: afterAttn[i] + mlpOut[i].",difficulty:"warmup",starterCode:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = new Array(x.length).fill(0);
  for (let i = 0; i < x.length; i++) {
    modulated[i] = x[i] * (1 + scale[i]) + shift[i];
  }
...
return results;`,hints:["Iterate over the elements.","Set afterMlp[i] to afterAttn[i] + mlpOut[i]."],solution:`function ditBlockStep(x, scale, shift, attnOut, mlpOut) {
  const modulated = new Array(x.length).fill(0);
  for (let i = 0; i < x.length; i++) {
    modulated[i] = x[i] * (1 + scale[i]) + shift[i];
  }
  ...
  return { modulated, afterAttn, afterMlp };
}`,explanation:"This completes one entire Diffusion Transformer block, incorporating the time conditioning, attention mechanism, and feed-forward MLP."}],J=[{id:"bloom-hash-index",stepLabel:"70.1",group:"Hash positions",title:"Bloom Filter Hash Function",concept:"Bloom filters use multiple hash functions to map items to bit array indices.",objective:"Compute the bit index for a string item using a simple seeded hash: sum(charCodes[i] * seed) % size.",difficulty:"warmup",starterCode:`function getBloomHash(item, seed, size) {
  let hash = 0;
...
return results;`,hints:["Scale the accumulated link mass by d, then add teleport.","nextRanks[i] = d * nextRanks[i] + teleport;"],solution:`function pagerankStep(ranks, adjList, d = 0.85) {
  ...
  return nextRanks;
}`,explanation:"Teleportation represents a surfer typing a random URL, ensuring convergence stability in disconnected web graphs."}],G=[{id:"frontier-weight-raw-params",stepLabel:"19.1",group:"Weight bytes",title:"Raw Parameter Count",concept:"Frontier LLM serving memory is dominated by parameters. First, we expand the billions shorthand to the absolute number of parameters.",objective:"Expand the billions count by multiplying by 10^9.",difficulty:"warmup",starterCode:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
...
return results;`,hints:["Multiply numParamsBillions by 1e9."],solution:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const absoluteParams = numParamsBillions * 1e9;
  
  const rawBytes = absoluteParams * bytesPerParam;
  return rawBytes / 1e9;
}`,explanation:"A 70B model technically means 70,000,000,000 floating point numbers in memory."},{id:"frontier-weight-raw-bytes",stepLabel:"19.2",group:"Weight bytes",title:"Raw Byte Size",concept:"Next, we compute the total memory footprint in raw bytes by multiplying the parameter count by the bytes per parameter.",objective:"Multiply absoluteParams by bytesPerParam.",difficulty:"warmup",starterCode:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const absoluteParams = numParamsBillions * 1e9;
...
check('Llama 70B FP16', getWeightBytesGB(70, 2), 140);
return results;`,hints:["Multiply absoluteParams by bytesPerParam."],solution:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const absoluteParams = numParamsBillions * 1e9;
  
  const rawBytes = absoluteParams * bytesPerParam;
  
  return rawBytes / 1e9;
}`,explanation:"At 16-bit precision, each parameter occupies 2 bytes. At 8-bit precision, each takes 1 byte."},{id:"frontier-weight-gb",stepLabel:"19.3",group:"Weight bytes",title:"Gigabyte Conversion",concept:"Server memory is measured in Gigabytes (GB). We must divide the raw byte count by 10^9 to convert it to GB.",objective:"Convert rawBytes into GBs.",difficulty:"warmup",starterCode:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const absoluteParams = numParamsBillions * 1e9;
  const rawBytes = absoluteParams * bytesPerParam;
...
check('Llama 70B FP16', getWeightBytesGB(70, 2), 140);
return results;`,hints:["Divide rawBytes by 1e9."],solution:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const absoluteParams = numParamsBillions * 1e9;
  const rawBytes = absoluteParams * bytesPerParam;
  ...
  return gigabytes;
}`,explanation:"Converting to GB puts the memory requirement in the same units used by GPU specs (e.g. 80GB for an A100)."},{id:"frontier-weight-cancel",stepLabel:"19.4",group:"Weight bytes",title:"Cancellation Insight",concept:"Notice that multiplying by 10^9 (billion scale) and dividing by 10^9 (GB scale) perfectly cancels out! We can simplify the entire calculation.",objective:"Replace the 3-step calculation with the simplified direct multiplication.",difficulty:"core",starterCode:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
...
check('Llama 70B FP16', getWeightBytesGB(70, 2), 140);
return results;`,hints:["Just multiply numParamsBillions by bytesPerParam."],solution:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
  const gigabytes = numParamsBillions * bytesPerParam;
  
  return gigabytes;
}`,explanation:"Because 1 billion parameters is exactly 1 GB per 1 byte-per-param, we can skip the intermediate 1e9 conversions entirely."},{id:"frontier-weight-full",stepLabel:"19.5",group:"Weight bytes",title:"Optimized Finalization",concept:"Return the simplified expression directly.",objective:"Return the directly calculated GB footprint.",difficulty:"warmup",starterCode:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
...
return results;`,hints:["Return numParamsBillions * bytesPerParam."],solution:`function getWeightBytesGB(numParamsBillions, bytesPerParam) {
...
}`,explanation:"KV cache size is the main bottleneck for long-context generation, often exceeding parameter sizes at large batch sizes."},{id:"moe-active-ratio",stepLabel:"20.1",group:"Active fraction",title:"Active Ratio",concept:"At Frontier scale, Mixture of Experts (MoE) models only activate a subset of experts per token to minimize computational costs (FLOPs). First, we find the routing ratio.",objective:"Calculate the active fraction of experts: activeExperts / totalExperts.",difficulty:"warmup",starterCode:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
...
return results;`,hints:["Divide activeExperts by totalExperts."],solution:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  
  const activeExpertParams = activeRatio * totalExpertParams;
  return nonAttnBase + activeExpertParams;
}`,explanation:"In an 8x7B model that activates 2 experts per token, the active ratio is 2/8 (or 25%)."},{id:"moe-active-expert-params",stepLabel:"20.2",group:"Active fraction",title:"Active Expert Footprint",concept:"We apply the routing ratio to the total expert parameters to find how many expert parameters are actually used during a single forward pass.",objective:"Multiply totalExpertParams by the activeRatio.",difficulty:"core",starterCode:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
...
check('Mixtral 8x7B active params', getActiveParams(12, 35, 2, 8), 20.75);
return results;`,hints:["Multiply totalExpertParams by activeRatio."],solution:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  
  const activeExpertParams = activeRatio * totalExpertParams;
  
  return nonAttnBase + activeExpertParams;
}`,explanation:"This drastically reduces the FLOPs required. You get the capacity of a massive model with the latency of a much smaller one."},{id:"moe-active-base",stepLabel:"20.3",group:"Active fraction",title:"Static Baseline",concept:"MoE layers only replace the Feed-Forward Network (FFN). The self-attention layers and embeddings (nonAttnBase) still run for every token.",objective:"Identify the static baseline parameters that must always be added.",difficulty:"warmup",starterCode:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  const activeExpertParams = activeRatio * totalExpertParams;
...
check('Mixtral 8x7B active params', getActiveParams(12, 35, 2, 8), 20.75);
return results;`,hints:["The baseline is nonAttnBase."],solution:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  const activeExpertParams = activeRatio * totalExpertParams;
  ...
  return baseline + activeExpertParams;
}`,explanation:"The attention mechanism isn't conditionally routed, so its parameter footprint is constant."},{id:"moe-active-total",stepLabel:"20.4",group:"Active fraction",title:"Total Active Parameters",concept:"The final active parameter count per token is the sum of the dense attention layers and the sparse routed experts.",objective:"Add the baseline to the active expert parameters.",difficulty:"core",starterCode:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  const activeExpertParams = activeRatio * totalExpertParams;
  const baseline = nonAttnBase;
...
check('Mixtral 8x7B active params', getActiveParams(12, 35, 2, 8), 20.75);
return results;`,hints:["Return baseline + activeExpertParams."],solution:`function getActiveParams(nonAttnBase, totalExpertParams, activeExperts, totalExperts) {
  const activeRatio = activeExperts / totalExperts;
  const activeExpertParams = activeRatio * totalExpertParams;
  const baseline = nonAttnBase;
  
  return baseline + activeExpertParams;
}`,explanation:"By activating only 2 out of 8 experts per token, MoE models keep latency low while offering massive model capacities."},{id:"mla-compression-bottleneck",stepLabel:"21.1",group:"Cache size ratio",title:"Latent Bottleneck",concept:"Multi-head Latent Attention (MLA) compresses the KV cache. First, we identify the dimension of the compressed latent space.",objective:"Extract the compressed latent dimension size.",difficulty:"warmup",starterCode:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
...
return results;`,hints:["Assign compressedSize to latentDim."],solution:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  
  const standardSize = kvHeads * headDim;
  return compressedSize / standardSize;
}`,explanation:"Instead of caching full keys and values for every head, MLA only caches this tiny latent representation."},{id:"mla-compression-dense",stepLabel:"21.2",group:"Cache size ratio",title:"Dense KV Size",concept:"To understand the savings, we need to know what the memory footprint would have been using standard Multi-Head Attention.",objective:"Calculate the uncompressed KV size: kvHeads * headDim.",difficulty:"core",starterCode:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
...
check('DeepSeek-V2 MLA ratio', getMLACacheRatio(128, 128, 128), 0.0078125);
return results;`,hints:["Multiply kvHeads by headDim."],solution:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  
  const standardSize = kvHeads * headDim;
  
  return compressedSize / standardSize;
}`,explanation:"A model with 128 heads of dimension 128 normally requires 16,384 floats per token just for the cache!"},{id:"mla-compression-math",stepLabel:"21.3",group:"Cache size ratio",title:"Compression Math",concept:"The ratio is exactly the new compressed size divided by the old standard size.",objective:"Compute the ratio of compressedSize to standardSize.",difficulty:"core",starterCode:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  const standardSize = kvHeads * headDim;
...
check('DeepSeek-V2 MLA ratio', getMLACacheRatio(128, 128, 128), 0.0078125);
return results;`,hints:["Divide compressedSize by standardSize."],solution:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  const standardSize = kvHeads * headDim;
  ...
  return ratio;
}`,explanation:"For DeepSeek-V2, this ratio reaches an incredible 1/128th of the original size (0.0078)."},{id:"mla-compression-full",stepLabel:"21.4",group:"Cache size ratio",title:"Full MLA Ratio",concept:"Return the computed MLA compression ratio.",objective:"Return the final ratio value.",difficulty:"warmup",starterCode:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  const standardSize = kvHeads * headDim;
  const ratio = compressedSize / standardSize;
...
check('DeepSeek-V2 MLA ratio', getMLACacheRatio(128, 128, 128), 0.0078125);
return results;`,hints:["Return the ratio."],solution:`function getMLACacheRatio(latentDim, kvHeads, headDim) {
  const compressedSize = latentDim;
  const standardSize = kvHeads * headDim;
  const ratio = compressedSize / standardSize;
  
  return ratio;
}`,explanation:"MLA reduces the memory footprint of KV caches by more than 90%, allowing much larger batch sizes and context lengths."},{id:"grpo-relative-advantage-sum",stepLabel:"GRPO.1",group:"Relative advantage",title:"Group Score Sum",concept:"Group Relative Policy Optimization (GRPO) calculates advantages relative to a group baseline. First, we need the sum of all scores in the group.",objective:"Compute the sum of all scores in the array.",difficulty:"warmup",starterCode:`function getRelativeAdvantages(scores) {
...
return results;`,hints:["Use reduce, e.g. scores.reduce((sum, s) => sum + s, 0);"],solution:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  ...
  return advantages;
}`,explanation:"The sum is the first step to finding the group baseline."},{id:"grpo-relative-advantage-mean",stepLabel:"GRPO.2",group:"Relative advantage",title:"Group Baseline Mean",concept:"The mean score acts as the baseline for the group. We subtract this from each individual score to find if it was above or below average.",objective:"Compute the mean by dividing the sum by n.",difficulty:"warmup",starterCode:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  const sum = scores.reduce((acc, s) => acc + s, 0);
...
return results;`,hints:["mean = sum / n;"],solution:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  const sum = scores.reduce((acc, s) => acc + s, 0);
  const mean = sum / n;
  ...
  return advantages;
}`,explanation:"Subtracting the mean centers the advantages around zero."},{id:"grpo-relative-advantage-center",stepLabel:"GRPO.3",group:"Relative advantage",title:"Centering Scores",concept:"By subtracting the mean from each score, we get raw centered advantages. Positive means better than average.",objective:"Inside the loop, push (scores[i] - mean) into the advantages array.",difficulty:"warmup",starterCode:`function getRelativeAdvantages(scores) {
  const n = scores.length;
...
return results;`,hints:["advantages.push(scores[i] - mean);"],solution:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  const mean = scores.reduce((sum, s) => sum + s, 0) / n;
  ...
  return advantages;
}`,explanation:"Centered scores act as unscaled advantages."},{id:"grpo-relative-advantage",stepLabel:"GRPO.4",group:"Relative advantage",title:"GRPO Advantage Calculation",concept:"To finalize the GRPO advantages, we divide the centered scores by the standard deviation. This normalizes the advantages.",objective:"Divide the centered score by std.",difficulty:"core",starterCode:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  const mean = scores.reduce((sum, s) => sum + s, 0) / n;
...
return results;`,hints:["advantage = (scores[i] - mean) / std;"],solution:`function getRelativeAdvantages(scores) {
  const n = scores.length;
  const mean = scores.reduce((sum, s) => sum + s, 0) / n;
  ...
  return advantages;
}`,explanation:"By standardizing scores within a group, GRPO avoids needing a learned value network, simplifying the RL pipeline."},{id:"budget-thinking-check-complete",stepLabel:"23.1",group:"Budget split",title:"Thinking Token Search",concept:"Test-time compute thinking models generate reasoning steps between <thought> and </thought> tags before emitting the final answer.",objective:"Check if the tokens array includes the </thought> tag.",difficulty:"warmup",starterCode:`function parseThinkingOutput(tokens) {
...
return results;`,hints:["Use tokens.includes('</thought>')"],solution:`function parseThinkingOutput(tokens) {
  ...
  return { isComplete, thoughtTokens, answerTokens };
}`,explanation:"Detecting the end tag allows the server to switch from reasoning mode to output generation mode."},{id:"budget-thinking-check-index",stepLabel:"23.2",group:"Budget split",title:"Find Thought Boundary",concept:"To split the output, we need to locate exactly where the reasoning ends.",objective:"Find the index of </thought> using indexOf().",difficulty:"warmup",starterCode:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
...
return results;`,hints:['endIndex = tokens.indexOf("</thought>");'],solution:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
  ...
  return { isComplete, endIndex, thoughtTokens, answerTokens };
}`,explanation:"Finding the index is required before slicing the array."},{id:"budget-thinking-check-thoughts",stepLabel:"23.3",group:"Budget split",title:"Extracting Thoughts",concept:"We can now isolate the internal thought process by slicing the array from the start tag to the end tag.",objective:"Slice the tokens array from index 1 to endIndex to get the thought tokens.",difficulty:"warmup",starterCode:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
  const endIndex = tokens.indexOf('</thought>');
...
return results;`,hints:["thoughtTokens = tokens.slice(1, endIndex);"],solution:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
  const endIndex = tokens.indexOf('</thought>');
  ...
  return { isComplete, thoughtTokens, answerTokens };
}`,explanation:"Test-time compute models use these thoughts internally to structure their final response."},{id:"budget-thinking-check",stepLabel:"23.4",group:"Budget split",title:"Extracting Final Answer",concept:"Once thinking is complete, any tokens emitted after the closing tag form the final user-facing response.",objective:"Slice the tokens array from endIndex + 1 to the end to get the answer tokens.",difficulty:"core",starterCode:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
  const endIndex = tokens.indexOf('</thought>');
...
return results;`,hints:["answerTokens = tokens.slice(endIndex + 1);"],solution:`function parseThinkingOutput(tokens) {
  const isComplete = tokens.includes('</thought>');
  const endIndex = tokens.indexOf('</thought>');
  ...
  return { isComplete, thoughtTokens, answerTokens };
}`,explanation:"By splitting thoughts and answers, the UI can render reasoning chains dynamically in an accordion while displaying the answer plainly."},{id:"long-context-scale-factor",stepLabel:"24.1",group:"Linear seq scaling",title:"Context Growth Factor",concept:"Long context models scale their KV cache memory footprint linearly with context length. The first step is to compute the growth factor.",objective:"Compute the growth factor: newLen / baselineLen.",difficulty:"warmup",starterCode:`function scaleMemory(baselineMB, baselineLen, newLen) {
...
return results;`,hints:["factor = newLen / baselineLen;"],solution:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
  const scaled = baselineMB * factor;
  return Math.floor(scaled);
}`,explanation:"The factor represents how many times larger the new sequence is compared to the baseline."},{id:"long-context-scale-direct",stepLabel:"24.2",group:"Linear seq scaling",title:"Direct Scaling Application",concept:"With the growth factor calculated, we apply it directly to the baseline memory footprint.",objective:"Multiply baselineMB by the factor.",difficulty:"warmup",starterCode:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
...
check('scale from 4k to 32k', scaleMemory(256, 4096, 32768), 2048);
return results;`,hints:["scaled = baselineMB * factor;"],solution:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
  const scaled = baselineMB * factor;
  return Math.floor(scaled);
}`,explanation:"Because attention memory scales linearly with context length (for KV cache), a 8x longer sequence requires 8x the memory."},{id:"long-context-scale-floor",stepLabel:"24.3",group:"Linear seq scaling",title:"Integer Memory Bounds",concept:"Memory allocations are often chunked. Using Math.floor ensures we provide a safe lower bound integer.",objective:"Apply Math.floor to the scaled memory.",difficulty:"warmup",starterCode:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
  const scaled = baselineMB * factor;
...
return results;`,hints:["return Math.floor(scaled);"],solution:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
  const scaled = baselineMB * factor;
  return Math.floor(scaled);
}`,explanation:"Bounding the memory usage prevents out-of-memory errors from floating point imprecision."},{id:"long-context-scale-kv",stepLabel:"24.4",group:"Linear seq scaling",title:"Linear sequence memory scaling",concept:"We can optimize the computation into a single step.",objective:"Combine the calculation into a single return statement: Math.floor(baselineMB * (newLen / baselineLen)).",difficulty:"core",starterCode:`function scaleMemory(baselineMB, baselineLen, newLen) {
  const factor = newLen / baselineLen;
  const scaled = baselineMB * factor;
...
check('scale from 4k to 32k', scaleMemory(256, 4096, 32768), 2048);
return results;`,hints:["return Math.floor(baselineMB * (newLen / baselineLen));"],solution:`function scaleMemory(baselineMB, baselineLen, newLen) {
  return Math.floor(baselineMB * (newLen / baselineLen));
}`,explanation:"Linear scaling shows how cache requirements grow directly with the input sequence length."},{id:"omni-fuse-vision-scale",stepLabel:"25.1",group:"Weighted fuse",title:"Vision Scaling",concept:"Omni models fuse text and vision tokens by applying gating layers. The gate value controls how much of the vision embedding to keep.",objective:"Scale the vision embedding: multiply visionEmb[i] by gate.",difficulty:"warmup",starterCode:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
...
return results;`,hints:["vScaled = gate * visionEmb[i];"],solution:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
  ...
  return fused;
}`,explanation:"The gating value allows the network to dynamically attend more to vision or text depending on the input context."},{id:"omni-fuse-text-scale",stepLabel:"25.2",group:"Weighted fuse",title:"Text Scaling",concept:"The remaining portion of the representation (1 - gate) is allocated to the text embedding.",objective:"Scale the text embedding: multiply textEmb[i] by (1 - gate).",difficulty:"warmup",starterCode:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
    const vScaled = gate * visionEmb[i];
...
return results;`,hints:["tScaled = (1 - gate) * textEmb[i];"],solution:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
    const vScaled = gate * visionEmb[i];
  ...
  return fused;
}`,explanation:"By strictly tying the two scales together (they sum to 1), the fusion operation preserves the overall magnitude of the activation."},{id:"omni-fuse-add",stepLabel:"25.3",group:"Weighted fuse",title:"Modality Addition",concept:"Now we merge the scaled embeddings together by adding them.",objective:"Add the scaled vision and text values and push them into the fused array.",difficulty:"warmup",starterCode:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
    const vScaled = gate * visionEmb[i];
    const tScaled = (1 - gate) * textEmb[i];
...
return results;`,hints:["fused.push(vScaled + tScaled);"],solution:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
    const vScaled = gate * visionEmb[i];
    const tScaled = (1 - gate) * textEmb[i];
  ...
  return fused;
}`,explanation:"This produces a single, combined sequence vector that can be processed by later transformer blocks natively."},{id:"omni-fuse-embeddings",stepLabel:"25.4",group:"Weighted fuse",title:"Weighted multimodal embedding fusion",concept:"We can optimize this fusion step into a single line inside the loop.",objective:"Fuse the embeddings directly: output[i] = gate * vision[i] + (1 - gate) * text[i].",difficulty:"core",starterCode:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
...
return results;`,hints:["fused.push(gate * visionEmb[i] + (1 - gate) * textEmb[i]);"],solution:`function fuseModalEmbeddings(textEmb, visionEmb, gate) {
  const fused = [];
  for (let i = 0; i < textEmb.length; i++) {
    fused.push(gate * visionEmb[i] + (1 - gate) * textEmb[i]);
  }
  return fused;
}`,explanation:"Many modern multimodal architectures, like Gemini, rely heavily on dynamic routing and gating mechanisms for cross-modality understanding."},{id:"diffusion-lm-mask-factor",stepLabel:"26.1",group:"Mask ratio",title:"Diffusion Noise Schedule",concept:"Diffusion language models iteratively denoise sequences. The noise schedule defines the fraction of tokens masked at time step t out of T.",objective:"Compute the masking fraction: t / T.",difficulty:"warmup",starterCode:`function getMaskCount(seqLen, t, T) {
...
return results;`,hints:["fraction = t / T;"],solution:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
  const rawCount = seqLen * fraction;
  return Math.round(rawCount);
}`,explanation:"The linear noise schedule masks progressively fewer tokens as generation proceeds from t=T down to t=0."},{id:"diffusion-lm-mask-raw",stepLabel:"26.2",group:"Mask ratio",title:"Sequence Fraction",concept:"We apply the masking fraction to the total sequence length to find how many tokens should be masked.",objective:"Multiply the seqLen by the fraction.",difficulty:"warmup",starterCode:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
...
return results;`,hints:["rawCount = seqLen * fraction;"],solution:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
  const rawCount = seqLen * fraction;
  return Math.round(rawCount);
}`,explanation:"This determines the theoretical continuous number of tokens to corrupt."},{id:"diffusion-lm-mask-round",stepLabel:"26.3",group:"Mask ratio",title:"Discrete Mask Count",concept:"Because we can only mask whole tokens, we round the raw count to the nearest integer.",objective:"Apply Math.round to the raw count.",difficulty:"warmup",starterCode:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
  const rawCount = seqLen * fraction;
...
return results;`,hints:["return Math.round(rawCount);"],solution:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
  const rawCount = seqLen * fraction;
  return Math.round(rawCount);
}`,explanation:"Rounding provides the exact discrete number of [MASK] tokens to insert into the input sequence."},{id:"diffusion-lm-mask-ratio",stepLabel:"26.4",group:"Mask ratio",title:"Diffusion language model mask ratio",concept:"We can optimize the computation into a single step.",objective:"Combine the calculation into a single return statement: Math.round(seqLen * (t / T)).",difficulty:"core",starterCode:`function getMaskCount(seqLen, t, T) {
  const fraction = t / T;
  const rawCount = seqLen * fraction;
...
check('mid diffusion step', getMaskCount(100, 50, 100), 50);
check('start diffusion step', getMaskCount(100, 90, 100), 90);
return results;`,hints:["return Math.round(seqLen * (t / T));"],solution:`function getMaskCount(seqLen, t, T) {
  return Math.round(seqLen * (t / T));
}`,explanation:"The noise schedule defines how many tokens are masked/corrupted at each stage of the diffusion generation process."},{id:"serving-batch-valid",stepLabel:"27.1",group:"Continuous batching",title:"Validate Active Slots",concept:"In continuous batching LLM serving, tokens are processed in shared batch slots. First, we must ensure the active slot count does not exceed the maximum capacity.",objective:"Return Math.min(activeSlots, maxBatchSlots) to ensure we do not exceed capacity.",difficulty:"warmup",starterCode:`function getBatchUtilization(activeSlots, maxBatchSlots) {
...
return results;`,hints:["validActive = Math.min(activeSlots, maxBatchSlots);"],solution:`function getBatchUtilization(activeSlots, maxBatchSlots) {
  const validActive = Math.min(activeSlots, maxBatchSlots);
  return validActive / (maxBatchSlots || 1);
}`,explanation:"Hardware limits the maximum parallel requests a GPU can serve."},{id:"serving-batch-zero",stepLabel:"27.2",group:"Continuous batching",title:"Handle Zero Capacity",concept:"If there are no batch slots available (e.g., node is offline), we must prevent division by zero.",objective:"If maxBatchSlots is 0, return 0.",difficulty:"warmup",starterCode:`function getBatchUtilization(activeSlots, maxBatchSlots) {
...
return results;`,hints:["if (maxBatchSlots === 0) return 0;"],solution:`function getBatchUtilization(activeSlots, maxBatchSlots) {
  if (maxBatchSlots === 0) return 0;
  const validActive = Math.min(activeSlots, maxBatchSlots);
  return validActive / maxBatchSlots;
}`,explanation:"Checking for zero capacity prevents NaN errors in system monitoring."},{id:"serving-batch-ratio",stepLabel:"27.3",group:"Continuous batching",title:"Calculate Ratio",concept:"To monitor system load, we divide the active slots by the total available slots.",objective:"Compute the utilization ratio: validActive / maxBatchSlots.",difficulty:"warmup",starterCode:`function getBatchUtilization(activeSlots, maxBatchSlots) {
  if (maxBatchSlots === 0) return 0;
  const validActive = Math.min(activeSlots, maxBatchSlots);
...
return results;`,hints:["return validActive / maxBatchSlots;"],solution:`function getBatchUtilization(activeSlots, maxBatchSlots) {
  if (maxBatchSlots === 0) return 0;
  const validActive = Math.min(activeSlots, maxBatchSlots);
  return validActive / maxBatchSlots;
}`,explanation:"A ratio near 1.0 indicates the hardware is fully saturated."},{id:"serving-batch-utilization",stepLabel:"27.4",group:"Continuous batching",title:"Serving batch slot utilization",concept:"We can shorten this logic using a ternary operator.",objective:"Combine into: maxBatchSlots === 0 ? 0 : Math.min(activeSlots, maxBatchSlots) / maxBatchSlots.",difficulty:"core",starterCode:`function getBatchUtilization(activeSlots, maxBatchSlots) {
...
return results;`,hints:["return maxBatchSlots === 0 ? 0 : Math.min(activeSlots, maxBatchSlots) / maxBatchSlots;"],solution:`function getBatchUtilization(activeSlots, maxBatchSlots) {
...
}`,explanation:"The stop flag is what lets an outer loop alternate between model generation and tool execution."},{id:"agentic-apply-presence",stepLabel:"30.1",group:"Hunk apply",title:"Verify target string",concept:"Agentic coding engines edit code bases by applying diff patches. Before replacing, the agent must verify the target block exists.",objective:"Return content unchanged when targetString is not found.",difficulty:"warmup",starterCode:`function applyPatch(content, targetString, replacementString) {
...
return results;`,hints:["if (!content.includes(targetString)) return content;"],solution:`function applyPatch(content, targetString, replacementString) {
  if (!content.includes(targetString)) {
    return content;
  }
  return content;
}`,explanation:"Strict matching ensures the AI agent does not accidentally modify the wrong section of a file."},{id:"agentic-apply-index",stepLabel:"30.2",group:"Hunk apply",title:"Locate target string",concept:"Finding the exact index of the target block helps debugging if there are multiple identical blocks.",objective:"Use indexOf and return content unchanged when the target is missing.",difficulty:"warmup",starterCode:`function applyPatch(content, targetString, replacementString) {
...
check('missing target unchanged', applyPatch('let x = 1;', 'let y = 1;', 'let y = 2;'), 'let x = 1;');
check('found target not yet replaced', applyPatch('let x = 1;\\nreturn x;', 'let x = 1;', 'let x = 2;'), 'let x = 1;\\nreturn x;');
return results;`,hints:["if (index === -1) return content;"],solution:`function applyPatch(content, targetString, replacementString) {
  const index = content.indexOf(targetString);
  if (index === -1) {
    return content;
  }
  return content;
}`,explanation:"Advanced code agents often use AST parsers or line-number tracking instead of simple string searches."},{id:"agentic-apply-error",stepLabel:"30.3",group:"Hunk apply",title:"Handle missing targets",concept:"If the target string cannot be found (perhaps due to a hallucination or an outdated read), the system should abort rather than silently corrupting the file.",objective:"Throw an Error when indexOf returns -1; otherwise return content unchanged.",difficulty:"warmup",starterCode:`function applyPatch(content, targetString, replacementString) {
  const index = content.indexOf(targetString);
...
return results;`,hints:["if (index === -1) throw new Error('Target not found');"],solution:`function applyPatch(content, targetString, replacementString) {
  const index = content.indexOf(targetString);
  if (index === -1) {
    throw new Error('Target not found');
  }
  return content;
}`,explanation:"Failing fast ensures human operators can review the conflict before it cascades."},{id:"agentic-apply-patch",stepLabel:"30.4",group:"Hunk apply",title:"Agentic text replacement",concept:"Once verified, the replacement block is swapped into the exact location of the target block.",objective:"Return the updated content using content.replace.",difficulty:"core",starterCode:`function applyPatch(content, targetString, replacementString) {
  const index = content.indexOf(targetString);
  if (index === -1) {
    throw new Error('Target not found');
  }
